@testitem "Measurements / internal shunt / correlated implicit sensitivities" tags=[:extension] begin
    ...
    values = E._shunt_values(domain,Formulation().methods,50one(epsilon),20one(epsilon))
    ...
    deterministic(eps) = begin
        ...
        E.internal_shunt_response(E._shunt_values(d,Formulation().methods,50.0,20.0),d;level).C
    end
    ...
    geometry_values = E._shunt_values(geometry_domain,Formulation().methods,50one(radius),20one(radius))
    ...
    angle_values = E._shunt_values(angle_domain,Formulation().methods,50one(wire_angle),20one(wire_angle))
