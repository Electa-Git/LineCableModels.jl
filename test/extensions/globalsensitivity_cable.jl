@testitem "GlobalSensitivity / Sobol / real cable-domain smoke" tags=[:extension] setup=[
    ...
    problem_space=LineParametersProblem(
        system;
        temperature = 20.0,
        earth_props = EarthModel(100.0, 10.0, 1.0),
        frequencies = [50.0]
    )
    ...
    point=only(collect(LineCableModels.points(problem.space)))
    descriptors=LineCableModels.uncertainties(point)
    @test length(descriptors) == 2
    nominal_arguments=LineCableModels.realize_arguments(
        point, map(nominal, descriptors)
    )
    nominal_problem=LineCableModels.realize(point, nominal_arguments)
    ...
    records=only(details(first_result).points)
