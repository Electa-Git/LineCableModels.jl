@testitem "Gmsh FEM / production edge paths / affine field and Stokes controls" tags=[:extension] begin
    ...
    @test integrate([(0.0, 0.0), (2.0, 1.0)]) ≈ 2a[1]+a[2]
    @test integrate([(0.0, 0.0), (2.0, 0.0), (2.0, 1.0), (0.0, 1.0), (0.0, 0.0)]) ≈ 2b*2
    @test integrate([(0.5, 0.0), (0.5, 1.0)]; active = [false, true]) ≈ (a[2]+0.5b)*0.75
    @test_throws ErrorException integrate([(0.0, 0.0), (0.0, 2.0)])
...
@testitem "Gmsh FEM / physics selection, native maps, and resume isolation" tags=[
    ...
    for value in (:quasi_tem, "quasi-tem", Symbol("quasi-tem"))
        @test formulation_options(LineCableModelsFEM, FormulationOptions(physics = value)).data.physics ===
              Symbol("quasi-tem")
    end
    ...
    @test tem.details.data.fem.inputs.options.physics !==
          fw.details.data.fem.inputs.options.physics
    ...
    try
        ...
        FEM._publish_transport!(run, model, joinpath(path, "input/model_data.pro"),
            joinpath(path, "mesh/model.msh"), last(formulations),
    ...
    mktempdir() do root
        ...
        lock(FEM.FEM_SESSION_LOCK) do
            ...
            try
                ...
                mesh = only(FEM._select_meshes!(info,
                    model,
                    geometry,
