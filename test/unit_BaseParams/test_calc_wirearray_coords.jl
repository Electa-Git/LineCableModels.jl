@testitem "BaseParams: calc_circstrands_coords unit tests" setup = [defaults] begin
	...
	@testset "Type Stability and Promotion" begin
		...
		@testset "Mixed types: Center C is Measurement" begin
			...
			@test Measurements.value(coords[1][1]) ≈ Measurements.value(C[1]) + lay_radius atol =
				TEST_TOL
			@test Measurements.value(coords[1][2]) ≈ Measurements.value(C[2]) atol =
				TEST_TOL
			@test Measurements.uncertainty(coords[1][1]) ≈ Measurements.uncertainty(C[1]) atol =
				TEST_TOL
			@test Measurements.uncertainty(coords[1][2]) ≈ Measurements.uncertainty(C[2]) atol =
				TEST_TOL
		end
	end
end
