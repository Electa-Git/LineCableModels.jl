...
@testitem "DataModel module" setup = [defaults, deps_datamodel] begin
    ...
    mktempdir(joinpath(@__DIR__)) do tmpdir
        ...
        LineCableModels.save(db_to_save, file_name = output_file)
    ...
    cable_core = cable_design.components[findfirst(c -> c.id == "core", cable_design.components)]
    cable_sheath = cable_design.components[findfirst(
        c -> c.id == "sheath",
        cable_design.components
    )] # Note: Tutorial used 'cable_shield' variable name
    cable_jacket = cable_design.components[findfirst(
        c -> c.id == "jacket",
        cable_design.components
    )]
    ...
    mktempdir(joinpath(@__DIR__)) do tmpdir # Create a temporary directory for the test file
        ...
        # Test saving
        @test isfile(LineCableModels.save(library, file_name = output_file))
    ...
    cablepos = CablePosition(cable_design, xa, ya, Dict("core" => 1, "sheath" => 0, "jacket" =>
        0))
    cable_system = LineCableSystem(cable_system_id, 1000.0, cablepos)
    add!(
        cable_system,
        cable_design,
        xb,
    ...
    mktempdir(joinpath(@__DIR__)) do tmpdir
        ...
        if !isnothing(params_cable1_node)
            ...
            if !isnothing(rhoc_val_str)
                ...
                expected_rhoc = cable_system.cables[1].design_data.components[1].conductor_props.rho
                println(
            ...
            if !isnothing(eps1_val_str)
                ...
                expected_eps1 = cable_system.cables[1].design_data.components[1].insulator_props.eps_r
                println(
        ...
        ground_params = findfirst("//User[@name='master:Line_Ground']/paramlist", project_node)
    ...
    if get(ENV, "LINECABLEMODELS_TEST_PLOTTING", "false") == "true"
        ...
        @test LineCableModels.PlotBuilder.BackendHandler.with_backend(
            () -> :rendered,
            :cairo
        ) == :rendered
