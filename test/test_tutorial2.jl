@testitem "examples/tutorial2.jl tests" setup = [defaults] begin
	...
	@testset "core and main insulation" begin
		...
		core = ConductorGroup(CircStrands(0.0, Diameter(d_w), 1, 0.0, material_al))
		add!(core, CircStrands, Diameter(d_w), 6, 15.0, material_al)
		add!(core, CircStrands, Diameter(d_w), 12, 13.5, material_al)
		add!(core, CircStrands, Diameter(d_w), 18, 12.5, material_al)
	...
	screen_con = ConductorGroup(
		CircStrands(main_insu, Diameter(d_ws), num_sc_wires, lay_ratio, material_cu),
	)
	...
	@testset "user error handling and robustness" begin
		...
		@test_throws ArgumentError CircStrands(0.0, Diameter(-1.0), 1, 0.0, material_al)
		@test_throws ArgumentError Insulator(core, Thickness(-1.0), material_pe)
		@test_throws ArgumentError CircStrands(core, Diameter(d_w), 1, -1.0, material_al) # Negative lay ratio
		...
		@test_throws ArgumentError CablePosition(
			cable_design,
			1.0,
			1.0,
			Dict("non_existent_component" => 1),
		)
		...
		f = 10.0 .^ range(0, stop = 6, length = 10)
		earth_params = EarthModel(f, 100.0, 10.0, 1.0)
		cablepos_partially_grounded = CablePosition(
			cable_design,
			xa,
			ya,
			Dict("core" => 1, "sheath" => 0, "jacket" => 0),
		)
		system_partially_grounded = LineCableSystem(
			"partially_grounded_system",
			1000.0,
			cablepos_partially_grounded,
		)
		mktempdir(joinpath(@__DIR__)) do temp_dir
			output_file = joinpath(temp_dir, "partially_grounded_export.pscx")
			result_path = export_data(
				:pscad,
				system_partially_grounded,
				earth_params,
