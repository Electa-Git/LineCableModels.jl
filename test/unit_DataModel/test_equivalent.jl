...
@testitem "simplify unit tests" setup = [
    ...
    @testset "Basic Functionality" begin
        ...
        for (orig, simp) in zip(des.components, des_s.components)
            @test simp.conductor_group.r_in ≈ orig.conductor_group.r_in atol = TEST_TOL
            @test simp.conductor_group.r_ex ≈ orig.conductor_group.r_ex atol = TEST_TOL
            @test simp.insulator_group.r_ex ≈ orig.insulator_group.r_ex atol = TEST_TOL
    ...
    @testset "Equivalence Preservation" begin
        ...
        for (orig, simp) in zip(des.components, des_s.components)
            ...
            @test simp.conductor_group.resistance ≈ orig.conductor_group.resistance atol = TEST_TOL
