...
@testitem "DataModel(ConductorGroup.add!): unit tests" setup =
	[defaults, deps_datamodel, defs_materials, defs_con_group] begin
	using Measurements
	...
	@testset "Combinatorial Type Testing (constructor path of added part)" begin
		...
		g = make_core_group()
		g3 = add!(g, CircStrands, m(rad_wire0, 1e-6), 6, 10.0, copper_props)
		@test eltype(g3) <: Measurement

		# Mixed case B: middle arg (lay_ratio) is Measurement
		g = make_core_group()
		g4 = add!(g, CircStrands, rad_wire0, 6, m(10.0, 0.1), copper_props)
