@testitem "ParametricBuilder(SystemBuilderSpec): combinatorics + value integrity" setup = [defaults] begin
    # -------------------------------------------------------------------------
    # Shared setup (mirrors your example, but we toggle `unc` per testset)
    # -------------------------------------------------------------------------
    using LineCableModels
    using LineCableModels.ParametricBuilder:
                                             CableBuilder, build, Conductor, Insulator,
                                             Material, Earth, SystemBuilder, at,
    ...
    function make_parts(
        ...
        return [
            # CORE conductors: stranded (central + rings) — uses PB coupling semantics.
            Conductor.Stranded(
                :core;
                layers = co_n,
                d = (co_d, unc),
    ...
    @testset "Baseline: fully deterministic (cardinality=1, value equality)" begin
        ...
        parts = make_parts(
            ms_al_uq, ms_cu, ms_pe, ms_xlpe, ms_sem1, ms_sem2, ms_polyacryl; unc)
        cbs = CableBuilder("NA2XS(FL)2Y_1000", parts; nominal = datasheet_info)

        # CableBuilder cardinality should be 1 (all scalars).
        @test length(cbs) == 1
        ...
        # SystemBuilder cardinality is designs × length × positions(dx,dy) × temperature × earth.
        @test length(spec) == 1
        ...
        # Check system contents: 3 cables, phase mapping intact.
        sys = prob.system
    ...
    @testset "System knobs: length & temperature grids + % expansion" begin
        ...
        parts = make_parts(
            ms_al_uq, ms_cu, ms_pe, ms_xlpe, ms_sem1, ms_sem2, ms_polyacryl; unc = nothing)
        cbs = CableBuilder("NA2XS(FL)2Y_1000", parts; nominal = datasheet_info)
        @test length(cbs) == 1
        ...
        # Pull system line_length via internal field (LineCableSystem constructor stores it as T).
        found_meas = false
        for pr in probs
            sys = pr.system
    ...
    @testset "Position axes: displacement grids & anchor % semantics" begin
        ...
        # Case B: anchor % (no displacement sweep) — (nothing, pct) on dx.
        specB = make_spec(cbs;
    ...
    @testset "Material % propagates into designs (ρ with % → Measurement in design tree)" begin
        ...
        ms_al_uq = Material(materials, "aluminum", rho = (1.0, (0.0, 5.0, 2)), mu_r = (
            1.0, (0.0, 5.0, 2)))
        ms_cu = Material(materials, "copper")
        ms_pe = Material(materials, "pe")
        ms_xlpe = Material(materials, "xlpe")
        ...
        # designs = 2×2 from ρ(%) × μ(%) on the same MaterialSpec (coupled across parts when equal tuples).
