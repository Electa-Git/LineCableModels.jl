...
@testitem "EarthProps / conversion / identity and uncertainty promotion" tags=[:unit] setup=[
    ...
    model=EP.EarthModel([50.0], 100.0, 10.0, 1.0)
    layer=last(model.layers)
    @test convert(EP.EarthModel{Float64}, model) === model
    @test convert(EP.EarthLayer{Float64}, layer) === layer

    promoted_model=convert(EP.EarthModel{Measurement{Float64}}, model)
    promoted_layer=convert(EP.EarthLayer{Measurement{Float64}}, layer)
    @test eltype(promoted_model) === Measurement{Float64}
    @test eltype(promoted_layer) === Measurement{Float64}
    @test value(promoted_model.layers[end].base_rho_g) == 100.0
    @test uncertainty(promoted_model.layers[end].base_rho_g) == 0.0
    @test value(promoted_layer.base_epsr_g) == 10.0
    @test uncertainty(promoted_layer.base_epsr_g) == 0.0
end

@testitem "EarthProps / properties / formulation layer and model contracts" tags=[:unit] setup=[
    DataModelTestSupport, UseDataModelSupport, TestNumerics, defs_earthprops] begin
    ...
    @testset "Mixed-type constructors & add!" begin
        ...
        @testset "add!: model T=Float64, add Measurement" begin
            ...
            modelF = @test_logs (:warn, r"created a new object") add!(
                modelF, freqsF, ρM, εM, μM; t = 12.0)
