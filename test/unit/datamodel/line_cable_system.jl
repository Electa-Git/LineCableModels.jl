...
@testitem "DataModel / LineCableSystem / constructor & add! unit tests" tags=[:unit] setup=[
    DataModelTestSupport, UseDataModelSupport, TestNumerics, defs_linesys] begin
    ...
    @testset "Promotion on add! (Float64 → Measurement)" begin
        ...
        sysP = @test_logs (:warn, r"promoted") DM.add!(sysF, posM)
...
@testitem "DataModel / LineCableSystem / promotion safety (intern-proof)" tags=[:unit] setup=[
    DataModelTestSupport, UseDataModelSupport, TestNumerics, defs_linesys] begin
    using Test
    using Measurements
