...
@testitem "DataModel / CableDesign / unit tests" tags=[:unit] setup=[
    DataModelTestSupport, UseDataModelSupport, TestNumerics, TestFixtures, MaterialFixtures, cable_fixtures] begin
    # -------------------------
    # Tests
    # -------------------------
    ...
    @testset "Basic Functionality (Float64)" begin
        g, ig = make_groups()
        @test g.r_ex ≈ ig.r_in atol = TestNumerics.absolute_floor(Float64)
