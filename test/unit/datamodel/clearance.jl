...
@testitem "DataModel / exterior clearance / touching, overlap and reconstruction" tags=[:unit] setup=[ClearanceFixtures] begin
    ...
    @test sum(p -> p.x, system.positions) ≈ sum(p -> p.x, system.input_positions) atol=1e-15
    @test sum(p -> p.y, system.positions) ≈ sum(p -> p.y, system.input_positions)
    @test getproperty.(declared, :pose) == system.input_positions
...
@testitem "DataModel / exterior clearance / propagated radii and correlated at coordinates" tags=[:extension] setup=[ClearanceFixtures] begin
    ...
    @test nominal(only(fixture.gaps(local_system))) >=
          nominal(local_system.clearances[1, 2])
    ...
    compensated_system=@test_logs (:warn, r"Cable placements adjusted") build(
        LineCableSystem,
...
@testitem "UQ / exterior clearance / retained budgets, extreme draws and bounded warnings" tags=[:integration] setup=[ClearanceFixtures] begin
    ...
    local_space=Gridspace{LineParametersProblem}(
        offset->begin
    ...
    problems=Gridspace{LineParametersProblem}(
        spacing->begin
        ...
        (Grid(0.021, AbsoluteError(0.002)),))
