@testsnippet defs_cablepos begin
    ...
    _out_radius(des) = max(
        des.components[end].conductor_group.r_ex,
        des.components[end].insulator_group.r_ex
    )
end

@testitem "DataModel / formations / flat spacing and orientation invariants" tags=[:unit] setup=[
    DataModelTestSupport,
    UseDataModelSupport
] begin
    horizontal=LineCableModels.DataModel.flat_formation(1.0, -2.0, 0.25)
    vertical=LineCableModels.DataModel.flat_formation(
        1.0,
        -2.0,
        0.25;
        vertical = true
    ...
    @test horizontal == (1.0, -2.0, 1.25, -2.0, 1.5, -2.0)
    @test vertical == (1.0, -2.0, 1.0, -2.25, 1.0, -2.5)
    @test diff(collect(horizontal[1:2:end])) == [0.25, 0.25]
    @test diff(collect(vertical[2:2:end])) == [-0.25, -0.25]
end

@testitem "DataModel / CablePosition / constructor unit tests" tags=[:unit] setup=[
    DataModelTestSupport, UseDataModelSupport, TestNumerics, defs_cablepos] begin
