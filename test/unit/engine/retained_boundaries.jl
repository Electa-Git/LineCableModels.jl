...
@testitem "ObservedResult / structured labels and equivalence conflicts" tags=[:unit] setup=[TestFixtures] begin
    ...
    @test occursin(description(Formulation().methods.earth_impedance;compact=true),labels[1])
    @test occursin(description(Formulation(earth_impedance=:pollaczek1926).methods.earth_impedance;compact=true),labels[2])
    ...
    @test occursin(description(Formulation().methods.earth_impedance;compact=true),with_external[1])
    @test occursin(description(Formulation(earth_impedance=:pollaczek1926).methods.earth_impedance;compact=true),with_external[2])
    fields=points[1].gridpoint.formulation_fields
    altered=merge(first(fields.Z),(summary="owned; punctuation",))
