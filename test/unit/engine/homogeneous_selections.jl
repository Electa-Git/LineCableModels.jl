@testitem "Engine / homogeneous selections retain each indexed formula through assembly" tags=[:unit] setup=[FormulaContractModels] begin
    ...
    # Manufactured nonzero potential coefficients exercise assembly without
    # inventing a built-in mixed-earth scientific model.
    ...
    design = build(CableDesign, "selection-contract", Stack(Group(:phase,
    ...
    problem = LineParametersProblem(system; earth_props = homogeneous(rho = 100.0), frequencies = [50.0, 500.0])
    choices = (
    ...
    for (indices, leaf) in ((1:2, choices.air), (3:4, choices.earth))
        ...
        subresult = compute(subproblem, Formulation(earth_impedance = leaf,
    ...
    @test provenance.requested.earth_impedance == (air = :Carson1926, earth = :Pollaczek1926, mixed = :Lucca1994)
    ...
    @test Set((record.formula, record.source, record.target) for record in records) ==
        Set(((:Carson1926, 1, 1), (:Pollaczek1926, 2, 2), (:Lucca1994, 1, 2), (:Lucca1994, 2, 1)))
    ...
    @test_throws ArgumentError compute(problem, Formulation(earth_impedance = merge(choices,
        (mixed = formula(:default),)), earth_admittance = potential))

    # Different configurations of the same source must not be merged by identity tag.
    calls = Tuple[]
    air_hook = (functor, pair, workspace) -> begin
        push!(calls, (:air, pair.layers, pair.row, pair.column))
        1.0 + 2.0im
    end
    earth_hook = (functor, pair, workspace) -> begin
        push!(calls, (:earth, pair.layers, pair.row, pair.column))
        3.0 + 4.0im
    end
    mixed_hook = (functor, pair, workspace) -> begin
        push!(calls, (:mixed, pair.layers, pair.row, pair.column))
        complex(pair.layers[1], pair.layers[2])
    end
    for (id, hook) in ((:default, air_hook), (:default, earth_hook), (:Lucca1994, mixed_hook))
        @eval LineCableModels.computation_options(::LineCableModels.FormulaMethod{$(QuoteNode(id)),
            typeof(E.EarthImpedance.earth_impedance)}, ::$(typeof(hook))) = (;)
    end
    modified = compute(problem, Formulation(earth_impedance = (
    ...
    @test details(modified).formulations.modified.earth_impedance == (air = true, earth = true, mixed = true)

    # Y has independent selections and hooks under exactly the same grammar.
    potential_calls = Symbol[]
    potential_hook = name -> (functor, pair, workspace) -> begin
    ...
    potential_choices = map((:air, :earth, :mixed)) do name
        ...
        @eval LineCableModels.computation_options(::LineCableModels.FormulaMethod{:ContractLayers,
            typeof(E.EarthAdmittance.earth_potential_coefficient)}, ::$(typeof(hook))) = (;)
    ...
    independent_y = compute(problem, Formulation(earth_impedance = choices,
        ...
        options = (ideal_transposition = false,)); options = (trace = true,))
...
@testitem "Engine / scalar and homogeneous shorthand preserve numerical and model contracts" tags=[:unit] setup=[TestFixtures] begin
    ...
    reordered = Formulation(earth_impedance = (mixed = same.mixed, earth = same.earth, air = same.air))
    ...
    model = build(E.EarthModel, (LineCableModels.Earth.EarthLayer(100.0, 10.0, 1.0, 0.5), LineCableModels.Earth.EarthLayer(200.0, 20.0, 1.0)))
    ...
    reduced = compute(layered, Formulation(
