@testitem "TextDisplay / calculation intent and numerical results" tags=[:unit] setup=[
    ...
    blueprints = EN.CableBlueprint{eltype(line_problem)}[
        EN.flatten(LineCableModelsCoaxial(), design, eltype(line_problem))
        for design in line_problem.system.designs]
    workspaces = map((false, true)) do trace
        execution = LineCableModels.Grammar.computation_options(LineCableModelsCoaxial, (; trace))
...
@testitem "TextDisplay / lazy parametric and uncertainty intent" tags=[:unit] setup=[
    ...
    earth_definition = formula(:Xue2021; order=:before, layer=2)
    soil_definition = formula(:CIGRE2019; epsilon_infinity=10.0)
    ...
    @test occursin("Xue2021", sprint(show, earth_definition))
    @test occursin("before", sprint(show, earth_definition))
    @test occursin("layer", sprint(show, earth_definition))
    @test occursin("epsilon_infinity", sprint(show, soil_definition))
...
@testitem "TextDisplay / report definitions do not publish or write" tags=[:unit] begin
    ...
    line = RB.LineParametersTableDefinition((@observe(R[:, :, :]),), :base, :kilo, nothing, true)
