@testitem "TextDisplay / calculation intent and numerical results" tags=[:unit] setup=[
    ...
    @test occursin("schelkunoff1934", shown)
    @test occursin("ametani1980", shown)
    @test occursin("semicon_admittance", shown)
    @test occursin("unified", shown)
    @test occursin("ametani1980", sprint(show, MIME"text/plain"(), cable_formulation))
