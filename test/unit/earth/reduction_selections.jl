@testitem "Analytical selections / explicit earth reduction preserves formula and indexed geometry" tags=[:unit] begin
    ...
    for height in (2.0, -1.0)
        ...
        for identifier in (:default, :unified), admittance_id in (:default, :unified)
            ...
            @test details(reduced).data.formulations.methods.earth_impedance.identifier ===
                (identifier === :default ? :unified : identifier)
            @test details(reduced).data.formulations.methods.earth_admittance.identifier ===
