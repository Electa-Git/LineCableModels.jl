@testitem "Gauntlet / PSCAD verified completed-run reuse" tags=[:gauntlet_toolkit] setup=[GauntletSupport] begin
    ...
    try
        ...
        design = build(CableDesign, "resume-fixture", terminal(:core,
        ...
        remote = P.RemoteConfig("fixture", GauntletSupport.WORK_ROOT, "unused", "julia", "python";
        ...
        @test_throws ArgumentError computation_options(P.PSCADFormulation, (; remote, solver_identity=1))
        ...
        files = [joinpath(root, file) for (root, _, names) in walkdir(source) for file in names]
        original = Dict(path=>(bytes2hex(open(sha256, path)), mtime(path)) for path in files)
        @test isfile(joinpath(source, "complete.toml"))
        reused = compute(problem, Formulation(:pscad; earth_impedance=:DirectNumericalIntegration); options)
        @test launches[] == 1
        ...
        @test details(reused).formulations.requested.earth_impedance == "DirectNumericalIntegration"
        total = compute(problem, Formulation(:pscad); options=(; options..., output_basis=:total))
        @test launches[] == 1
        @test Z(total) == 42 .* Z(first_result)
        @test Y(total) == 42 .* Y(first_result)
        @test original == Dict(path=>(bytes2hex(open(sha256, path)), mtime(path)) for path in files)
        callbacks = Int[]
        batch = compute(problem, [Formulation(:pscad), Formulation(:pscad; internal_impedance=:Schelkunoff1934)];
            options=(; options..., on_result=(problem, index, result)->push!(callbacks, index)))
        @test launches[] == 1
        @test callbacks == [1, 2]
        @test details(batch[1]).formulations.requested.internal_impedance == "default"
        @test details(batch[2]).formulations.requested.internal_impedance == "Schelkunoff1934"
        @test Z(batch[1]) == Z(batch[2])
        @test Z(batch[1]) !== Z(batch[2])
        @test occursin("no solver execution", details(batch[2]).execution.elapsed_scope)
        changed_earth = LineParametersProblem(system; earth_props=homogeneous(rho=200.0), frequencies=frequency)
        @test_throws ArgumentError compute(changed_earth, Formulation(:pscad);
            options=(; remote, resume_run_directory=source))
        changed_frequencies = LineParametersProblem(system; earth_props=homogeneous(rho=100.0),
            frequencies=collect(10.0 .^ range(-1, 5; length=101)))
        @test_throws ArgumentError compute(changed_frequencies, Formulation(:pscad);
            options=(; remote, resume_run_directory=source))

        @test_throws ArgumentError compute(problem, Formulation(:pscad; earth_impedance=:Saad1996);
            options=(; remote, resume_run_directory=source))
