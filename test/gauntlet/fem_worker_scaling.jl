...
const original=LineCableModels.ImportExport.deserialize_value(JSON3.read(read(joinpath(source,"input/problem.json"),String)))
const recorded=JSON3.read(read(joinpath(source,"input/computation.json"),String))
const indices=[1,15,29,43,58,72,86,101]
const problem=LineParametersProblem(original.system;frequencies=original.frequencies[indices],
...
for workers in (1,2,4,8)
    ...
    meshes=[joinpath(source,"mesh",i==101 ? "model.msh" : @sprintf("frequency_%04d.msh",i)) for i in indices]
    println("SCALING_START workers=",workers," run=",run.path);flush(stdout)
    peak_worker=Ref(0.0);peak_aggregate=Ref(0.0);sampled=Ref(0.0)
    function sample_memory()
        Sys.islinux() && time()-sampled[]>0.1 || return true
        sampled[]=time();aggregate=0.0
        ...
        for directory in readdir(attempts;join=true)
            ...
            peak_worker[]=max(peak_worker[],rss);aggregate+=rss
        end
    ...
    E._write_scan_checksums(run,scan);E._transition!(run,E.completed,"worker scaling validation completed")
    peak=0.0
    events=Dict(key=>0 for key in ("MatLUFactorSym","MatLUFactorNum","MatSolve","KSPSolve"))
    for attempt in readdir(joinpath(run.path,"attempts");join=true)
        for found in eachmatch(r"Mem\s*=\s*([0-9.]+)\s*Mb",read(joinpath(attempt,"getdp.log"),String))
    ...
    push!(reports,report);E._write_json_atomic(joinpath(root,"report.json"),reports)
    println(JSON3.write(report));flush(stdout)
end
