@testitem "Gauntlet / UQ campaign preflight and scientific settings" tags=[:gauntlet_toolkit] setup=[GauntletSupport] begin
    ...
    @test owner.semantic_sha256(reshape(vector, 2, 2)) !=
          owner.semantic_sha256(reshape(vector, 1, 4))
...
@testitem "Gauntlet / UQ campaign results, exact replay and partial resumption" tags=[:gauntlet] setup=[GauntletSupport] begin
    ...
    mktempdir() do temporary
        ...
        @test owner.run_campaign(root, [:two_bare_wires]; backends = (:coaxial,),
            catalogue = false, choices, propagation = (
                :deterministic, :linear_error, :monte_carlo),
        ...
        model=owner.campaign_models([:two_bare_wires], variation)[(
            :two_bare_wires, true)]
        @test model.problem isa Gridspace
        @test has_uncertainty(model.problem)
        @test owner.campaign_input(model, :linear_error) ==
              owner.campaign_input(model, :monte_carlo)
        for method in (:linear_error, :monte_carlo)
            job=only(filter(job->job["propagation"]==string(method), plan["jobs"]))
            records=[JLD2.load(joinpath(root, job["id"], lpad(index, 4, '0')*".jld2"))
                     for index in 1:2]
            ...
            @test owner.numerical_input_sha256(restored) ==
                  owner.numerical_input_sha256(model.nominal_problem)
            @test records[1]["formulation"].definitions.earth_impedance === :default
            @test length(records[1]["frequencies"]) == 101
            @test records[1]["correlation"].rule === :parameter_identity
            for (index, selection) in pairs(job["selections"])
                inner=owner.campaign_formulation(:coaxial, selection, :default)
                direct=compute(ParametricProblem(model.problem),
                    owner.campaign_propagation(method, :coaxial, inner, plan["monte_carlo"]))
                @test records[index]["moments"] ==
                      NamedTuple(owner.extract_moments(direct, model.port_order))
            end
        ...
        partial_plan["jobs"][1]["selections"][2]["earth_impedance"]="Carson1926"
        ...
        @test !owner.resume_campaign(root)
        @test [(sha256(read(file)), stat(file).mtime) for file in files] == before
        @test only(filter(row -> endswith(row.id, "monte_carlo"), owner.campaign_status(root))).state ==
              "failed"
    end
end
