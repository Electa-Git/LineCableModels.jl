    export ARTIFACT_ROOT, ARTIFACTS_TOML, ISHEADLESS, SNAPSHOT_SCHEMA_VERSION,
           ...
           read_collection, run_case, run_snapshot, snapshot_path,
    ...
    function _semantic_formulation_record(record)
        ...
        values = Dict(string(key) => value for (key, value) in pairs(record)
            if Symbol(key) ∉ (:type, :assumptions))
    ...
    # Type-valued metadata is a leaf, not a Julia runtime object graph.
    _same_problem_structure(left::Type, right::Type) = left === right
