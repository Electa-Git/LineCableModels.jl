@testitem "Gauntlet / manual campaign checkpoints and resume" tags=[:gauntlet] begin
    using SHA
    using JLD2
    ...
    mktempdir() do temporary
        ...
        # A later selection can fail without discarding an earlier result from
        # the same Gridspace batch. Repeated failure must not overwrite it.
        partial = joinpath(temporary, "partial")
        mkpath(partial)
        _, plan = owner.campaign_plan(root)
        push!(plan["jobs"][1]["selections"], Dict("id"=>"unsupported_coaxial",
            "earth_impedance"=>"DirectNumericalIntegration", "earth_admittance"=>"default"))
...
@testitem "Gauntlet / manual selections use the complete formulation grammar" tags=[:gauntlet_toolkit] begin
    using LineCableModels
    include(joinpath(pkgdir(LineCableModels), "test", "gauntlet", "runner.jl"))
    owner = GauntletSupport
    choices = owner.parse_selections(["--select", "insulation_admittance=default,Ametani2004",
    ...
    @test [(value.insulation_admittance, value.semicon_admittance) for value in paired.selections] ==
        [(:default, :default), (:Ametani2004, :Ametani2004)]
    @test Set((value.insulation_admittance, value.semicon_admittance) for value in product.selections) ==
    ...
    alternatives = owner.parse_selections(["--select", "internal_impedance=default,Ametani2004",
        "--select", "earth_properties=default,CIGRE2019",
        "--select", "equivalent_earth=default,Xue2021",
        "--select", "pipe_impedance=default"])
    selected = owner.campaign_selections(nothing, :coaxial, false; choices=alternatives, combine=:zip)
    @test length(selected.selections) == 2
    @test selected.selections[2].earth_properties === :CIGRE2019
    @test selected.selections[2].equivalent_earth === :Xue2021
    ...
    for arguments in (["--select"], ["--select", "earth_impedance"],
            ...
            ["--select", "earth_impedance=default", "--select", "earth_impedance=Xue2018"])
        @test_throws ArgumentError owner.parse_selections(arguments)
    end
end

@testitem "Gauntlet / explicit formulation axes checkpoint without replacement" tags=[:gauntlet] begin
    using SHA
    using JLD2
    using LineCableModels
    include(joinpath(pkgdir(LineCableModels), "test", "gauntlet", "runner.jl"))
    ...
    mktempdir() do temporary
        ...
        files = [joinpath(root, "two_bare_wires_coaxial", lpad(index, 4, '0') * ".jld2") for index in 1:2]
        ...
        expected = compute(model.nominal_problem, Formulation(
...
@testitem "Gauntlet / CI cannot start or resume a manual campaign" tags=[:gauntlet_toolkit] begin
    using LineCableModels
    include(joinpath(pkgdir(LineCableModels), "test", "gauntlet", "runner.jl"))
    mktempdir() do root
...
@testitem "Gauntlet / FEM checkpoints fingerprint executable bytes" tags=[:gauntlet_toolkit] begin
    using LineCableModels
    include(joinpath(pkgdir(LineCableModels), "test", "gauntlet", "runner.jl"))
    mktempdir() do root
