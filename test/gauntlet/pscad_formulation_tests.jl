@testitem "Gauntlet / PSCAD shared grammar and constitutive export" tags=[:gauntlet_toolkit] setup=[GauntletSupport] begin
    ...
    @test P.formulation_record(Formulation(:pscad)).effective.internal_impedance === :default
...
@testitem "Gauntlet / PSCAD consumes complete homogeneous choices and preserves export settings" tags=[:gauntlet_toolkit] setup=[GauntletSupport] begin
    using EzXML
    const P = GauntletSupport.PSCADBenchmarks
    ...
    staged = P._stage_pscad_project(problem, selected, setting)
    try
    ...
    record = P.formulation_record(selected)
    @test record.requested.earth_impedance == choices
    @test record.effective.earth_admittance === :default
    @test record.raw[:selections].earth_impedance.air.identifier === :Gary1976
