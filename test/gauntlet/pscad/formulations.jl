...
function earth_impedance(::Val{ID}, ::Val{Kind}, ::Val{S}, ::Val{T}, ::Val{:pscad}) where {ID, Kind, S, T}
...
function earth_potential_coefficient(::Val{ID}, ::Val{Kind}, ::Val{S}, ::Val{T}, ::Val{:pscad}) where {ID, Kind, S, T}
...
function earth_impedance(::Val{:default}, ::Union{Val{:self}, Val{:mutual}}, ::Val{1}, ::Val{1}, ::Val{:pscad})
    (EarthForm2 = (value = 2, readback = "DIRECT_NUMERICAL_INTEGRATION"),)
end
function earth_impedance(::Val{:default}, ::Union{Val{:self}, Val{:mutual}}, ::Val{2}, ::Val{2}, ::Val{:pscad})
...
function earth_impedance(::Val{:Gary1976}, ::Union{Val{:self}, Val{:mutual}}, ::Val{1}, ::Val{1}, ::Val{:pscad})
    (EarthForm2 = (value = 0, readback = "DERISEMLYEN"),)
end
function earth_impedance(::Val{:Carson1926}, ::Union{Val{:self}, Val{:mutual}}, ::Val{1}, ::Val{1}, ::Val{:pscad})
    (EarthForm2 = (value = 2, readback = "DIRECT_NUMERICAL_INTEGRATION"),)
end
function earth_impedance(::Val{:Pollaczek1926}, ::Union{Val{:self}, Val{:mutual}}, ::Val{2}, ::Val{2}, ::Val{:pscad})
    (EarthForm = (value = 2, readback = "DIRECT_NUMERICAL_INTEGRATION"),)
end
function earth_impedance(::Val{:WedepohlWilcox1973}, ::Union{Val{:self}, Val{:mutual}}, ::Val{2}, ::Val{2}, ::Val{:pscad})
    (EarthForm = (value = 0, readback = "WEDEPOHL"),)
end
function earth_impedance(::Val{:Saad1996}, ::Union{Val{:self}, Val{:mutual}}, ::Val{2}, ::Val{2}, ::Val{:pscad})
    (EarthForm = (value = 3, readback = "SAAD"),)
end
function earth_impedance(::Val{:Ametani2009}, ::Val{:mutual}, ::Val{1}, ::Val{2}, ::Val{:pscad})
    (EarthForm3 = (value = 0, readback = "AMETANIL"),)
end
function earth_impedance(::Val{:Ametani2009}, ::Val{:mutual}, ::Val{2}, ::Val{1}, ::Val{:pscad})
    (EarthForm3 = (value = 0, readback = "AMETANIL"),)
end
function earth_impedance(::Val{:Lucca1994}, ::Val{:mutual}, ::Val{1}, ::Val{2}, ::Val{:pscad})
    (EarthForm3 = (value = 2, readback = "LUCCA"),)
end
function earth_impedance(::Val{:Lucca1994}, ::Val{:mutual}, ::Val{2}, ::Val{1}, ::Val{:pscad})
    (EarthForm3 = (value = 2, readback = "LUCCA"),)
end
...
function earth_potential_coefficient(::Val{:default}, ::Union{Val{:self}, Val{:mutual}}, ::Val{1}, ::Val{1}, ::Val{:pscad})
    (;)
end
function earth_potential_coefficient(::Val{:default}, ::Union{Val{:self}, Val{:mutual}}, ::Val{2}, ::Val{2}, ::Val{:pscad})
    (;)
end
function earth_potential_coefficient(::Val{:default}, ::Val{:mutual}, ::Val{1}, ::Val{2}, ::Val{:pscad})
    (;)
end
function earth_potential_coefficient(::Val{:default}, ::Val{:mutual}, ::Val{2}, ::Val{1}, ::Val{:pscad})
    (;)
end
function internal_impedance(::Val{:default}, ::Union{Val{:inner}, Val{:outer}, Val{:mutual}}, ::Val{:pscad})
...
function formulas(owner::Module, kind::Val, source::Val, target::Val)
    equation = owner === EarthImpedance ? earth_impedance :
        owner === EarthAdmittance ? earth_potential_coefficient : throw(ArgumentError(
    ...
    return Tuple(id for id in owner.formulas() if
        which(equation, Tuple{Val{id}, typeof(kind), typeof(source), typeof(target), Val{:pscad}}) !== fallback)
end
...
function pscad_setting(formulation::PSCADFormulation, problem::LineParametersProblem)
    ...
    !model.vertical_layers && length(model.layers) == 2 &&
        ...
        "PSCAD Cable_Coax supports physical air and one homogeneous horizontal soil half-space"))
    ...
    (relation === nothing || relation === LineCableModels.Earth.FrequencyDependent.Formula(:default)) ||
        throw(ArgumentError("PSCAD does not implement the selected frequency-dependent soil relation"))
    for name in (:internal_impedance, :insulation_impedance, :earth_impedance, :earth_admittance)
    ...
    interactions = map(formulation.methods[(:earth_impedance, :earth_admittance)]) do selection
        ...
        records = NamedTuple{(:formula, :kind, :source, :target), Tuple{Symbol, Symbol, Int, Int}}[]
        for pair in pairs
            validate(pair)
            ...
            record = (formula = formula_id(selected), kind = pair.row == pair.column ? :self : :mutual,
                source = pair.layers[1], target = pair.layers[2])
            record in records && continue
            push!(records, record)
            for (field, setting) in Base.pairs(binding(Val(:pscad)))
                haskey(settings, field) && settings[field] != setting && throw(ArgumentError(
    ...
    for kind in kinds
        FormulaMethod(Val(formula_id(formulation.methods.internal_impedance)), internal_impedance, Val(kind))(Val(:pscad))
    end
    ...
    ground = (
        EarthForm2 = get(settings, :EarthForm2, (value = 2, readback = "DIRECT_NUMERICAL_INTEGRATION")),
        EarthForm = get(settings, :EarthForm, (value = 2, readback = "DIRECT_NUMERICAL_INTEGRATION")),
        EarthForm3 = get(settings, :EarthForm3, (value = 2, readback = "LUCCA")))
    return (ground = ground, frequency = (enablf = (value = 1, readback = "YES"),), interactions = interactions)
end
