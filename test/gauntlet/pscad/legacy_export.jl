# Private adaptation of main at 88f5de54; this is not a public exporter.
function export_legacy_pscad(
    ...
    format_nominal = (X; sigdigits = 4, minval = -1e30, maxval = 1e30) -> begin
        local_value = round(nominal(X), sigdigits = sigdigits)
    ...
    num_cables = ncables(cable_system)
    dx = 400
    for i in 1:num_cables
        ...
        components = collect(cable_position.design_data.components)
    ...
    ground_params_vector = [ # Renamed variable
        ...
        ("GRRES", format_nominal(earth_layer.rho)),
        ("GPERM", format_nominal(earth_layer.mu_r)),
        ("K0", "0.001"), ("K1", "0.01"), ("alpha", "0.7"),
        ("GRP", format_nominal(earth_layer.eps_r))
    ]
    ...
    write(file_name, doc)
    isfile(file_name) || error("legacy PSCAD exporter did not create $file_name")
    return file_name
end
