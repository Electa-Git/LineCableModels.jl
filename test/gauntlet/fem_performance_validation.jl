...
# Run from the repository root with --project=test/gauntlet and root on LOAD_PATH.
...
const ROOT = joinpath(pwd(), ".linecablemodels", "fem", "validation-" * Dates.format(now(), "yyyymmdd-HHMMSS"))
mkpath(ROOT)
println("VALIDATION_ROOT ", ROOT); flush(stdout)
const REPORTS = Any[]
    ...
    for basis in 1:n
        ...
        for (matrix,path) in zip(matrices,(paths.Z,paths.P))
            for row in filter(!isempty,strip.(readlines(path)))
                ...
                matrix[parse(Int,fields[3]),basis]=complex(parse(Float64,fields[5]),parse(Float64,fields[6]))
            end
        end
    end
    return matrices
...
function validate_campaign(name,method)
    ...
    problem=LineCableModels.ImportExport.deserialize_value(JSON3.read(read(joinpath(source,"input/problem.json"),String)))
    ...
    nf=length(problem.frequencies); n=length(model.terminal_ids)
    meshes=[joinpath(source,"mesh",f==nf ? "model.msh" : @sprintf("frequency_%04d.msh",f)) for f in 1:nf]
    oldrun=E.FEMRun(source,E.completed,"retained reference",:retained,"")
    reference=[E._parse_raw_matrix(Float64,joinpath(source,"raw",q*".tsv"),problem.frequencies,n,oldrun) for q in ("Z","P")]
    checked=falses(nf)
    errors=Dict(key=>0.0 for key in ("Z_relative","P_relative","Y_primitive_relative","Z_reduced_relative","P_reduced_relative","Y_reduced_relative","Z_absolute","P_absolute","Y_reduced_absolute"))
    residual=Ref(0.0); condition=Ref(0.0); entry_count=Ref(0)
    started=time(); last_print=Ref(0.0)
    function check_progress()
        for f in 1:nf
            checked[f] && continue
            all(b->isfile(E._column_paths(run.path,f,b,false).checkpoint),1:n) || continue
            z,p=read_frequency(run,f,n); zr=reference[1][:,:,f]; pr=reference[2][:,:,f]
            ...
            reduced=LineCableModels.Engine.reduce_primitive_matrices(reshape(z,n,n,1),reshape(p,n,n,1),problem.system.connection_order,form.options)
            ref_reduced=LineCableModels.Engine.reduce_primitive_matrices(reshape(zr,n,n,1),reshape(pr,n,n,1),problem.system.connection_order,form.options)
            ...
            for (key,a,b) in (("Z_reduced",reduced.Z,ref_reduced.Z),("P_reduced",reduced.P,ref_reduced.P),("Y_reduced",inversion.Y,ref_inversion.Y))
                errors[key*"_relative"]=max(errors[key*"_relative"],relative(a,b))
            end
            errors["Y_reduced_absolute"]=max(errors["Y_reduced_absolute"],maximum(abs.(inversion.Y-ref_inversion.Y)))
            residual[]=max(residual[],maximum(inversion.residuals))
            condition[]=max(condition[],maximum(inversion.condition_numbers))
            checked[f]=true; entry_count[]+=2n*n+length(inversion.Y)
        end
        if time()-last_print[]>=15 || all(checked)
            status=(source=name,run=run.path,checked_frequencies=count(checked),total_frequencies=nf,checked_entries=entry_count[],errors,
                max_inversion_residual=residual[],max_reduced_P_condition=condition[],elapsed_seconds=time()-started,
                launches=run.getdp_invocations,complete=all(checked),passed=all(checked)&&maximum(v for (k,v) in errors if endswith(k,"relative"))<1e-8)
            E._write_json_atomic(joinpath(ROOT,name*"-progress.json"),status)
            println(JSON3.write(status));flush(stdout);last_print[]=time()
        end
        return true
    end
    ...
    parameters=E._line_parameters(run,model,form,E._fem_computation_options((trace=true,)),scan,inputs)
    E._write_scan_checksums(run,scan);E._transition!(run,E.completed,"full numerical validation completed")
    @assert all(checked)
    report=JSON3.read(read(joinpath(ROOT,name*"-progress.json"),String),Dict{String,Any})
    ...
    println("CAMPAIGN_PASS ",name," ",JSON3.write(errors));flush(stdout)
end

for (name,method) in (("run-YiqIRh",:default),("run-aZVWtq",:Ametani2004),("run-Pbj2uz",:Ametani2004),("run-orkY1H",:default))
    validate_campaign(name,method)
end
println("ALL_FOUR_CAMPAIGNS_PASS ",ROOT);flush(stdout)
