@testitem "Gauntlet / comparison entry point resolves provenance and live catalogue" tags=[:gauntlet_toolkit] setup=[TestFixtures] begin
    ...
    @test provenance.implementation.selection.schema_version == 1
end
...
@testitem "Gauntlet / catalogue applicability follows formula-owned preflight" tags=[:gauntlet_toolkit] setup=[TestFixtures] begin
    ...
    @test isnothing(reason(:earth_impedance, :Xue2018))
    @test occursin("self impedance", reason(:earth_impedance, :Bridges1995))
    @test occursin("overhead", reason(:earth_impedance, :Wise1934))
    @test occursin("exactly two earth layers", reason(:earth_impedance, :Papadopoulos2011))
    ...
    layered_workspace = prepare_case(layered_model)
    @test isnothing(case_skip_reason(layered_model,
        choose(:earth_impedance, :Papadopoulos2011), layered_workspace))
    @test isnothing(case_skip_reason(layered_model,
        choose(:earth_admittance, :Papadopoulos2011), layered_workspace))
    ...
    @test isnothing(case_skip_reason(overhead_model,
        choose(:earth_impedance, :Wise1934), overhead_workspace))
    @test isnothing(case_skip_reason(overhead_model,
        choose(:earth_impedance, :Sunde1968), overhead_workspace))
    @test occursin("underground", case_skip_reason(overhead_model,
        choose(:earth_impedance, :Xue2018), overhead_workspace))
    ...
    @test occursin("nonzero horizontal", case_skip_reason(vertical_model,
        choose(:earth_impedance, :Saad1996), vertical_workspace))
    @test isnothing(case_skip_reason(vertical_model,
        choose(:earth_impedance, :Xue2018), vertical_workspace))

    calls = Ref(0)
    failing_kernel = (functor, pair) -> (calls[] += 1; throw(DomainError(0, "numerical sentinel")))
    overridden = merge(choose(:earth_impedance, :Bridges1995),
        (earth_impedance=formula(:Bridges1995; mutual=failing_kernel),))
    ...
    @test length(many_workspace.invariants.earth_pairs) == 6
    @test occursin("self impedance", case_skip_reason(multiple,
        choose(:earth_impedance, :Bridges1995), many_workspace))
...
@testitem "Gauntlet / manual PSCAD benchmark selections use registered formulations" tags=[:gauntlet_toolkit] setup=[GauntletSupport] begin
    ...
    for path in sort(paths)
        @testset "$(basename(path))" begin
            ...
            blocks = filter(node -> node isa Expr && node.head === :macrocall &&
                first(node.args) === Symbol("@testitem"), parsed.args)
            @test !isempty(blocks)
            for block in blocks
                declarations = Dict(node.args[1] => node.args[2] for node in last(block.args).args
