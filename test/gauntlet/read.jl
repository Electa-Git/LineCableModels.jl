...
function _snapshot_document(path::AbstractString, collection::Symbol)
    ...
    !isempty(expected_digest) && first(expected_digest) == observed_digest || throw(ArgumentError(
...
function read_moments(path::AbstractString; collection::Symbol=:uq)
    loaded = _snapshot_document(path, collection)
    get(loaded.snapshot, "comparison_policy", nothing) === :uq_moments ||
...
function read_collection(path::AbstractString; collection::Symbol)
    ...
    isdir(benchmarks) || throw(ArgumentError("collection has no benchmarks directory: $path"))
    directories = sort!(filter(isdir, readdir(benchmarks; join=true)))
    isempty(directories) && throw(ArgumentError("collection has no recorded benchmarks: $path"))
    return map(directories) do directory
        file = joinpath(directory, "snapshot.jld2")
        policy = JLD2.load(file, "comparison_policy")
        policy === :line_parameters && return _line_snapshot(file, collection)
        policy === :uq_moments && return read_moments(file; collection)
        throw(ArgumentError("unsupported stored comparison policy $policy: $file"))
    end
end
