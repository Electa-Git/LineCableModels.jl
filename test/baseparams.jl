@testitem "BaseParams module" setup = [defaults, defs_materials] begin
    ...
    @testset "Parallel impedance calculations" begin
        ...
        @test calc_parallel_equivalent(10.0, 5.0) ≈ (10.0 * 5.0) / (10.0 + 5.0) atol = TEST_TOL
    ...
    @testset "Resistance calculations" begin
        ...
        @test calc_tubular_resistance(r_in, r_ext, rho, alpha, T0, T) ≈ rho / area_tube atol = TEST_TOL
        # Solid conductor
        area_solid = π * r_ext^2
        # Solid conductor resistance (r_in=0)
        @test calc_tubular_resistance(0.0, r_ext, rho, alpha, T0, T) ≈ rho / area_solid atol = TEST_TOL
        ...
        @test calc_strip_resistance(thickness, width, rho, alpha, T0, T) ≈ rho / area_strip atol = TEST_TOL
    ...
    @testset "Helical parameters correction" begin
        ...
        @test overlength ≈ sqrt(1 + (π * mean_diam_expected / pitch_expected)^2) atol = TEST_TOL
    ...
    @testset "GMR calculations & consistency" begin
        ...
        gmr_array = calc_circstrands_gmr(layout_rad, num_wires, wire_rad, mu_r)
        # Single wire case should match solid wire GMR
        @test gmr_array > 0

        gmr_single_circ_strands = calc_circstrands_gmr(0.0, 1, wire_rad, mu_r) # Layout radius irrelevant for N=1
        gmr_single_wire_solid = calc_tubular_gmr(wire_rad, 0.0, mu_r)
        # GMR of 1-wire array matches solid wire GMR
        @test gmr_single_circ_strands ≈ gmr_single_wire_solid atol = TEST_TOL
    end
    ...
    @testset "GMD and equivalent GMR" begin
        ...
        part3_circstrands = CircStrands(0.03, 0.002, 7, 10.0, aluminum_props) # Wire array, separate
        ...
        # Let's test Tubular vs CircStrands where sub-elements exist
        gmd_1_3 = calc_gmd(part1_solid, part3_circstrands) # Should be approx layout_radius of part3 (0.03 + 0.002) if part1 is at center
        ...
        gmr_eq_expected = (core.gmr^(beta^2)) * (layer2.gmr^((1 - beta)^2)) *
        ...
        gmr_eq2_expected = (core.gmr^(beta2^2)) * (layer3_wa.gmr^((1 - beta2)^2)) *
    ...
    @testset "Inductance calculations" begin
        ...
        @test calc_tubular_inductance(r_in, r_ext, 2.0 * mu_r) ≈ 2.0 * L_expected atol = TEST_TOL #  Check mu_r scaling
        ...
        S = 0.1
        L_trifoil = calc_inductance_trifoil(
            r_in_co, r_ext_co, copper_props.rho, copper_props.mu_r,
        ...
        L_trifoil_S2 = calc_inductance_trifoil(
            r_in_co, r_ext_co, copper_props.rho, copper_props.mu_r,
    ...
    @testset "Capacitance & conductance" begin
        ...
        @test calc_shunt_conductance(r_in, r_ext, rho_ins / 10) ≈ 10 * G_expected atol = TEST_TOL
