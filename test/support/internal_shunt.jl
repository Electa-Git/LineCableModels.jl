...
function internal_shunt_test_design(;radius=.0003,epsilon=3.0,tapes=true,count=6,
        suffix="",core_radius=.002,wire_angle=.17)
    metal=Material(kind=:conductor,rho=2e-8,mu_r=1.0,T0=20.0,alpha=.004)
    ...
    if tapes
        push!(open_parts,terminal(Symbol(:middle,suffix),
            Group(:strip_screen,Region(:strip,Rectangle(.0012,t),metal);
                pattern=Ring(1;r=screen_outer+t/2))))
    end
    origin=Stack(terminal(Symbol(:inner,suffix),Region(:axial,Disk(core_radius),metal)),
        Region(:inner_dielectric,Shell(.002),insulation_material),
        Enclosure(:screen_domain,Stack(open_parts);primitive=Annulus(a,b),fill=host_material),
        Region(:outer_dielectric,Shell(.0008),insulation_material),
        terminal(Symbol(:reference,suffix),Region(:closed_screen,Shell(.0002),metal)),
        Region(:cover,Shell(.0007),insulation_material))
...
function internal_shunt_test_domain(design)
    ...
    domains=owner.internal_shunt_domains([design],[blueprint])
