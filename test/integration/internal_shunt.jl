@testitem "Engine / internal shunt / explicit boundary workflow and reuse" tags=[:integration] begin
    ...
    boundary = formula(:boundary;options=(resolution=(wire=32,order=16,quadrature=128,modes=256),))
    first_formula = Formulation(shunt_model=boundary,earth_impedance=:pollaczek1926,
        earth_admittance=:pollaczek1926;options=physical)
    other_formula = Formulation(shunt_model=boundary,earth_impedance=:saad1996,
        earth_admittance=:pollaczek1926;options=physical)
    reduced_formula = Formulation(shunt_model=boundary,earth_impedance=:pollaczek1926,
    ...
    lossy = Formulation(insulation_admittance=:lossy,
    ...
    @test CableConstants(pair;formulation=CableConstantsFormulation(shunt_model=boundary)).C ≈ fill(constants.C[1],2) rtol=1e-8
end
