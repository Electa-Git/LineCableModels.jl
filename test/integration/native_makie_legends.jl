@testitem "Makie addons / construction-time panel legends preserve source ownership" tags=[:visual] begin
    ...
    options = (; backend = :cairo, display_plot = false, controls = false,
        ...
        legend_cap = 0.5, length_unit = :base, quantity_units = :base,
    ...
    configured = Makie.plot(sources, requests; options...,
        panel_legends = Dict(
            (1, 1) => nothing,
            (1, 2) => (position = :top, max_fraction = 0.5,
    ...
    for rendered in (plot, inside, configured)
        ...
        @test [axis.title[] for axis in rendered.axes] ==
              ["Self resistance", "Mutual resistance"]
...
@testitem "Makie addons / independent scale arrangement and sibling spacing" tags=[:visual] setup=[TestFixtures] begin
    ...
    @testset "placement chooses default cells independently of bar orientation" begin
        ...
        @test all(bounds[i + 1].origin[1]>bounds[i].origin[1]+bounds[i].widths[1]
        for i in 1:2)
    end
...
@testitem "Makie addons / guide restoration retains native edits and common placement" tags=[:visual] setup=[TestFixtures] begin
    ...
    q=Makie.plot(raw, raw, raw, raw; ydata = ((R, 1, 1, :),),
        ...
        legend_position = :right, fig_size = (1200, 700))
    original=q.legend
...
@testitem "Makie addons / guide creation order shares complete scale geometry" tags=[:visual] setup=[TestFixtures] begin
    ...
    @test all(isapprox.(first_legend.figure.scene.viewport[].widths,
        first_scale.figure.scene.viewport[].widths; atol = 1))
    @test all(isapprox.(only(first_legend.axes).scene.viewport[].widths,
        only(first_scale.axes).scene.viewport[].widths; atol = 1))
    ...
    hidden=preview(design; backend = :cairo, display_plot = false,
        controls = false, display_colorbars = false)
...
@testitem "Makie addons / guide arrangement packing and option validation" tags=[:visual] begin
    ...
    for (extents, gap, span, fractions) in (([20.0, 30.0], 12.0, 62.0, [0.5, 0.5]),
        ...
        @test all(result.starts[i + 1]>=result.starts[i]+extents[i]+gap
...
@testitem "Makie addons / guide arrangement reflows retained complete items" tags=[:visual] setup=[TestFixtures] begin
    ...
    options=(backend = :cairo, display_plot = false, controls = false, size = (1400, 950),
        legend_position = nothing, colorbar_attributes = (
            vertical = false, width = 160, height = 14),
        colorbar_group_attributes = (
            layout = (1, 3), rowgap = 12.5, colgap = 16.5, margin = (0, 0, 0, 0)))
    ...
    Makie.limits!(only(p.axes), -0.03, 0.025, -0.025, 0.03)
    ...
    coordinates() = [Tuple(Makie.GridLayoutBase.gridcontent(item.layout).span.rows)[1]=>
                     Tuple(Makie.GridLayoutBase.gridcontent(item.layout).span.cols)[1]
                     for item in guide.items if item.visible[]]
    for position in (:right, :bottom, :top, :left, :bottom)
        ...
        @test all(a===b for (a, b) in zip(bars, p.colorbars))
        @test all(a===b
        for (a, b) in zip(companions, [item.companion for item in guide.items]))
...
@testitem "Makie addons / guide arrangement measures visible scales and independent gaps" tags=[:visual] setup=[TestFixtures] begin
    ...
    options=(backend = :cairo, display_plot = false, controls = false, size = (1500, 1100),
        colorbar_attributes = (vertical = false, width = 160, height = 14))
    p=preview(design; options..., legend_position = :bottom,
        legend_attributes = (halign = 0.25, margin = (0, 0, 0, 0)),
    ...
    function check_items(p, dimension, minimum_gap)
        ...
        for i in 1:(length(bounds) - 1)
            gap=dimension==1 ? bounds[i + 1].low[1]-bounds[i].high[1] :
                bounds[i].low[2]-bounds[i + 1].high[2]
    ...
    check_items(p, 1, 16.5)
    figurecolorbars!(p; position = :right, group_attributes = (
        layout = (3, 1), rowgap = 12.5, valign = 0.25))
    figurelegend!(p; position = :right, valign = 0.25, guide_spacing = (rowgap = 14.5,))
...
@testitem "Makie addons / scale bars share edges and baselines independently of labels" tags=[:visual] setup=[TestFixtures] begin
    ...
    function aligned(p, dimension, indices = eachindex(p.colorbars))
        Makie.colorbuffer(p.figure)
        bounds=[p.colorbars[i].layoutobservables.computedbbox[]
                for i in indices if p.colorbars[i].blockscene.visible[]]
    ...
    @test all(all(isapprox.(a, b; atol = 1))
    for (a, b) in zip(local_origins(p), local_origins(direct)))
    ...
    for position in (:bottom, :right, :bottom)
        figurecolorbars!(p; position, group_attributes = (layout = position===:bottom ?
                                                                   (1, 3) : (3, 1),))
...
@testitem "Makie addons / guide arrangement membership native edits and export" tags=[:visual] setup=[TestFixtures] begin
    ...
    design=TestFixtures.coaxial_design()
    options=(backend = :cairo, display_plot = false, controls = false,
        size = (1400, 1000), open_export = false)
    ...
    @test_throws ErrorException figurecolorbars!(
        hidden; position = :bottom, ticks = [1.0, 2.0],
    ...
    bars[3].labelsize[]=21
    Makie.limits!(only(p.axes), -0.024, 0.036, -0.025, 0.035)
    view=only(p.axes).targetlimits[]
    native=(size = p.figure.scene.viewport[], labels = [bar.label[] for bar in bars],
        fonts = [bar.labelfont[] for bar in bars], labelsize = [bar.labelsize[]
                                                                for bar in bars],
