@testitem "DataModel / cable constants / datasheet and physical invariants" tags=[:integration] setup=[
    ...
    separated=compute!(
        CableConstantsProblem(design; separation = 2.5*get_outer_radius(design)),
        Formulation()
    )
...
@testitem "Fixtures / factories / mutable state is never shared" tags=[:integration] setup=[
    ...
    @test first_system.num_cables == second_system.num_cables == 3
    @test first_system.num_phases == second_system.num_phases == 3
