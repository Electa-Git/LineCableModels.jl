...
function Base.NamedTuple(value::MomentResult)
    ...
    return (
        values = value.values, frequencies = copy(value.frequencies), basis = value.basis,
...
function Engine.compare(
    ...
    errors = map(reference.values, candidate.values) do left, right
        ...
        map(left, right) do a, b
            error=compare(a, b)
            RMSError{eltype(a)}(error.absolute,
                error.relative;
                details = merge(error.details,
                    (band = :all, actual_bounds = extrema(reference.frequencies),
                        indices = collect(eachindex(reference.frequencies)), reason = nothing)))
        end
    end
...
function moment_comparison_passes(comparison::MomentBenchmark, tolerances::NamedTuple)
    ...
    for statistic in (:mean, :std), quantity in keys(_MOMENT_QUANTITIES)
        ...
        all((error.absolute .<= limit.absolute) .|
            coalesce.(error.relative .<= limit.relative, false)) || return false
    end
    return true
end

function moment_comparison_passes(comparisons::AbstractVector, tolerances::NamedTuple)
    return all(comparisons) do row
        limit=getproperty(getproperty(tolerances, row.statistic), row.quantity)
        all((row.error.absolute .<= limit.absolute) .|
            coalesce.(row.error.relative .<= limit.relative, false))
    end
end
...
function moment_error_summary(comparison::MomentBenchmark)
    return map(comparison.errors) do product
        map(product) do error
            ...
            available=findall(!ismissing, error.relative)
            relative_index = isempty(available) ? nothing :
                             available[argmax(error.relative[available])]
            (
                maximum_absolute = error.absolute[absolute_index],
                absolute_location = Tuple(absolute_index),
                maximum_relative = relative_index === nothing ? missing :
                                   error.relative[relative_index],
                relative_location = relative_index === nothing ? nothing :
                                    Tuple(relative_index)
            )
        end
    end
end
    ...
    return map(comparison.errors, mean_limits, std_limits) do product, mean_limit, std_limit
        ...
        map(product, limits) do error, limit
            gating_ratio = min.(
                ...
                coalesce.(error.relative ./ limit.relative, Inf)
            )
            index = argmax(gating_ratio)
            visible = findall(error.absolute .> limit.absolute)
            available=collect(skipmissing(error.relative[visible]))
            relative = isempty(available) ? missing : maximum(available)
            (
