function build_case(::Val{:cable_380kv_no_armour_ac_flat}, p)
    ...
    design = LineCableModels.build(
        ...
        "cable_380kv_no_armour",
        LineCableModels.Stack(parts);
        nominal_data = (
            designation_code = "cable_380kv_no_armour",
            U0 = p.voltage / sqrt(3),
            U = p.voltage,
    ...
    connections = [Dict(:core => 2index - 1, :sheath => 2index) for index in 1:3]
    system = LineCableModels.build(
        ...
        connections,
        environment = earth,
        system_id = "cable_380kv_no_armour_ac_flat",
...
case_definition(Base.Fix1(build_case, Val(:cable_380kv_no_armour_ac_flat)),
    ...
    description = "380 kV unarmoured cables in AC flat formation"
)
