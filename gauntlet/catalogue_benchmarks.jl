...
function _catalogue_geometry_variation(definition::CaseDefinition,
    ...
    names = Tuple(p.id
    for p in values(definition.parameters)
    if _matches(p, (:geometry, :cable_layer)))
    isempty(names) &&
        throw(ArgumentError("catalogue case has no geometric uncertainty inputs"))
    ...
    for p in base
        all(
            value -> value isa Real && isfinite(value) && value > 0 &&
                     iszero(LineCableModels.uncertainty(value)),
            values(p)) || throw(ArgumentError(
    ...
    if definition.id === :cable_220kv_milliken_1x2500_252_trefoil
    ...
    independent_names = Tuple(name
    for (name, role) in zip(names, roles) if role !== :length)
    has_lengths = :length in roles
    primitive_names = ((has_lengths ? (:length_scale,) : ())..., independent_names...)
    inputs = NamedTuple{primitive_names}(ntuple(
        _ -> (
            nominal = 1.0, std = 0.1, support = _CATALOGUE_SCALE_SUPPORT, distribution = :uniform,
            interpretation = :multiplicative_factor),
        length(primitive_names)))
