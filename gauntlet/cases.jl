...
function _validate_loaded_problem(
        definition::CaseDefinition,
        nominal_problem::Engine.LineParametersProblem
)
...
function load_case(
    ...
    nominal_problem isa Engine.LineParametersProblem ||
        throw(ArgumentError(
    ...
    selected = Symbol[parameter.id
                      for (parameter, source) in
                          zip(values(definition.parameters), values(sources))
