...
function default_connect_options()
    (
        ...
        tls_server_name = get(ENV, "NATS_TLS_SERVER_NAME", nothing),
...
function validate_connect_options(server_info::Info, options)
    ...
    if get(options, :tls_required, false)
        tls_available = something(server_info.tls_required, false) ||
            something(server_info.tls_available, false)
        tls_available || error("Client requires TLS but it is not available for the server.")
    end
end
            ...
            if status(nc) == CONNECTING
                ...
                try
                    ...
                    if (@atomic nc.drain_event.set) == true
                        @debug "Connection attempt interrupted by an intentional drain" elapsed=time_diff exception=err
                        status(nc, DRAINING)
                        _do_drain(nc, false)
                        status(nc, DRAINED)
                    else
