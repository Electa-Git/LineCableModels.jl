...
function _do_drain(nc::Connection, is_connected::Bool; timeout::Union{Real, Period} = nc.drain_timeout)
    ...
    while !is_send_buffer_flushed(nc)
        ...
        if !is_connected
            # Draining a connection that never came online is an intentional
            # local shutdown path. There is no remote peer to flush to and an
            # error-level message here incorrectly reports normal cleanup as a
            # failure. Callers still receive publication errors at submission
            # time; the drain merely releases local resources.
            @debug "Skipping send-buffer flush while draining a disconnected connection"
            break
        end
        sleep(nc.drain_poll)
    end
