...
import LineCableModels.PlotBuilder.BackendHandler: current_backend_symbol, make_screen
...
const ICON_TTF = joinpath(
    pkgdir(@__MODULE__), "assets", "fonts", "material-icons", "MaterialIcons-Regular.ttf")
...
function PlotBuildArtifacts(; axis = nothing, legends = nothing, colorbars = nothing,
...
function build_backend_context(
        ...
        window = nothing,
        screen = nothing,
        icons::Function = (icon; text = nothing, kwargs...) -> (text === nothing ?
                                                                string(icon) :
                                                                string(text)),
    ...
    is_interactive = interactive === nothing ? backend in (:gl, :wgl, :wglmakie) :
                     interactive
    chan = statusbar
...
function _make_window(
    ...
    is_interactive = interactive_override === nothing ? actual_backend in (:gl, :wgl) :
...
function _run_plot_pipeline(
    ...
    buttons, toggles = _make_ctlbar!(fig_ctx, backend_ctx, button_specs, artifacts.control_toggles)
