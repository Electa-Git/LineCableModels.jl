...
function _interpolate_gradient(colors::Vector{<:Colorant}, t::Real)
    n = length(colors)
...
function _overlay(a::Colors.RGBA, b::Colors.RGBA)
...
function preview(design::CableDesign;
    ...
    fig = isnothing(axis) ? Makie.Figure(size = size, figure_padding = (10, 10, 10, 10)) :
          nothing
    ...
    if isnothing(axis)
        ...
        ax.title = display_id ? "Cable design preview: $(design.cable_id)" :
...
function preview(system::LineCableSystem;
    ...
    radii = Float64[(comp = last(c.design_data.components);
