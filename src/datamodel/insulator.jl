...
Represent one insulating layer at material reference temperature.

$(TYPEDFIELDS)
"""
struct Insulator{T <: Real} <: AbstractInsulatorPart{T}
    r_in::T
    r_ex::T
...
function Insulator(r_in::Real, r_ex::Real, material::Material)
    ...
    Validation.check(Insulator, (; r_in = rin, r_ex = rex))
    return validate(Insulator(
        rin,
        rex,
        props,
