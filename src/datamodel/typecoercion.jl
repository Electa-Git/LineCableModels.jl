...
function coerce_to_T(p::AbstractConductorPart{S}, ::Type{T}) where {S, T}
...
@inline coerce_to_T(p::AbstractInsulatorPart{S}, ::Type{T}) where {
    S, T} = _rebuild_part_typed_core(p, T)
