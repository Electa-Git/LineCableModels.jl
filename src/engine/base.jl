...
function _would_clip_any(x, tol)
...
function Base.show(io::IO, ::MIME"text/plain", rv::ResultsView)
    ...
    if rv.mode isa AsZY
        ...
        @inbounds for k in 1:nf
            ...
            fk = lp.f[k]
            ω = 2 * pi * float(fk)
            any_clipped |= _any_clipped(Zk; tol = tol, map = x -> scale * real(x)) ||
                           _any_clipped(Zk; tol = tol, map = x -> (scale * 1e3 / ω) *
                                                                  imag(x)) ||
                           _any_clipped(Yk; tol = tol, map = x -> scale * real(x)) ||
                           _any_clipped(Yk; tol = tol, map = x -> (scale * 1e6 / ω) *
                                                                  imag(x))
            any_clipped && break
        end
    end
    ...
    header_plain = @sprintf("%dx%dx%d LineParameters |  mode = %s  |  units per %s  |  tol = %s%s",
