...
function make_fem_problem!(
    ...
    fem_formulation.problem.filename = fem_formulation isa AbstractImpedanceFormulation ?
                                       workspace.paths[:impedance_file] :
                                       workspace.paths[:admittance_file]
...
function define_domain_groups!(
    ...
    for tag in keys(workspace.physical_groups)
        if tag > 10^8
            ...
            surface_type, entity_num, component_num, material_group,
            _ = decode_physical_group_tag(tag)
...
function run_getdp(workspace::FEMWorkspace, fem_formulation::AbstractFormulationSet)
    ...
    n_phases = sum([length(c.design_data.components)
...
function _do_run_solver!(freq_idx::Int,
    ...
    workspace.Z[:, :, freq_idx] = read_results_file(formulation.analysis_type[1], workspace)
    workspace.Y[:, :, freq_idx] = read_results_file(formulation.analysis_type[2], workspace)
end
