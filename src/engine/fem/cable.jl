...
function _make_cablepart!(workspace::FEMWorkspace, part::AbstractCablePart,
    ...
    mesh_size_current = _calc_mesh_size(
        r_in, r_ex, part.material_props, num_elements, workspace)
    ...
    next_part = layer_idx < num_layers ?
    ...
    if r_in ≈ 0
        # Solid disk
        _, _, marker, _ = draw_disk(
            x_center, y_center, r_ex, mesh_size, num_points_circumference)
    else
        # Annular shape
...
function _make_cablepart!(workspace::FEMWorkspace, part::CircStrands,
    ...
    is_single_wire = (num_wires == 1) &&
                     (isnothing(next_part) || !(next_part isa CircStrands))
    ...
    wire_positions = _calc_circstrands_coords(num_wires, r_in, r_ex, C = (
        x_center, y_center))
    ...
    for (wire_idx, (wx, wy)) in enumerate(wire_positions)
        _, _, marker, _ = draw_disk(
            wx, wy, radius_wire - TOL, mesh_size, num_points_circumference)
...
function _make_cablepart!(workspace::FEMWorkspace, part::Sector,
    ...
    # Calculate mesh size for this part
    num_elements = workspace.formulation.elements_per_length_conductor
    mesh_size = _calc_mesh_size(
        part.r_in, part.r_ex, part.material_props, num_elements, workspace)
    ...
    # Create physical tag
    physical_group_tag = encode_physical_group_tag(
        1, cable_idx, comp_idx, material_group, material_id)
...
function _make_cablepart!(workspace::FEMWorkspace, part::SectorInsulator,
    ...
    outer_vertices_translated = [Point(v[1] + x_center, v[2] + y_center)
                                 for v in part.outer_vertices]
    inner_vertices_translated = [Point(v[1] + x_center, v[2] + y_center)
                                 for v in part.inner_sector.vertices]
    ...
    surface_tag, marker = draw_polygon_with_hole(
        outer_vertices_translated, inner_vertices_translated, mesh_size)
...
function _make_cablepart!(workspace::FEMWorkspace, part::Tubular,
    ...
    if r_in ≈ 0
        ...
        air_physical_group_tag = encode_physical_group_tag(
            1, cable_idx, comp_idx, 2, air_material_id)
