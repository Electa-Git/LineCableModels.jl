...
function draw_polygon(vertices::Vector{<:Point})
    ...
    # Create lines
    lines = [gmsh.model.occ.add_line(points[i], points[i % length(points) + 1])
             for i in 1:length(points)]
...
function draw_polygon_with_hole(outer_vertices::Vector{<:Point},
        inner_vertices::Vector{<:Point}, max_edge_length::Number)
    ...
    outer_lines = [gmsh.model.occ.add_line(outer_points[i], outer_points[i % length(outer_points) + 1])
    ...
    inner_lines = [gmsh.model.occ.add_line(inner_points[i], inner_points[i % length(inner_points) + 1])
