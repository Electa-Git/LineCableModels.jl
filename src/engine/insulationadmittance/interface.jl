...
Each formula implements `insulation_material(selected, material, frequency,
...
$(TYPEDFIELDS)
"""
struct Formula{ID, P <: NamedTuple, O <: NamedTuple} <: InsulationAdmittanceFormulation
...
function Formula(::Val{ID}; parameters::NamedTuple=(;), options::NamedTuple=(;)) where {ID}
    ...
    binding = FormulaMethod(selected, insulation_material)
