...
Select an earth-impedance recipe by its literature identifier.
...
Store the frequency-dependent values shared by one earth-impedance recipe.
...
"Return the stable literature identifier of an earth-impedance formula."
formula_id(::Formula{ID}) where {ID} = ID

"Return the concrete leaf routes of an earth-impedance formula."
routes(formula::Formula) = formula.routes

"Return the physical assumptions of an earth-impedance formula."
...
Construct an earth-impedance formula from a literature identifier.
...
Construct a fully specified earth-impedance formula.
...
function _longitudinal(
    ...
    if propagation(formula) === Val(:zero) && !iszero(value)
        throw(ArgumentError(
            "earth-impedance formula :$(formula_id(formula)) fixes Γ to zero"
        ))
    end
