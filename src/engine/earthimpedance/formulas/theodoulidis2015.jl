function routes(::Val{:Theodoulidis2015})
    (
        self = theodoulidis2015,
        mutual = theodoulidis2015,
        Γ = theodoulidis2015_gamma
    )
end
...
theodoulidis2015_gamma(jω, permeability, permittivity) = (Γ = zero(jω), squared = zero(jω))
...
"""
function struve_h1(z::Complex{T}) where {T <: Real}
    term = 2z^2 / (3 * (one(T) * π))
    value = term
...
@inline function theodoulidis_term(u)
    return π / (2u) * (struve_h1(u) - bessely(1, u)) - inv(u^2)
end

function theodoulidis2015(functor, pair)
    _require(pair, Val(:overhead))
    state = functor.state
    geometry = _geometry(pair)
    u_1 = state.gamma[2] * (geometry.H - im * geometry.y_ij)
    u_2 = state.gamma[2] * (geometry.H + im * geometry.y_ij)
    correction = theodoulidis_term(u_1) + theodoulidis_term(u_2)
