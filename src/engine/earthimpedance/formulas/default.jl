...
function Formula(selected::Formula{:default}, ::Val{:overhead})
    resolved = Formula(Val(:Wise1934); selected.routes...)
    return Formula(Val(:Wise1934), resolved.routes,
...
function Formula(::Formula{:default}, ::Val{Placement}) where {Placement}
    throw(ArgumentError(
        "earth-impedance :default is not yet implemented for $Placement placement; " *
...
**Identification.** Problem-dependent default earth-impedance selection.

**Expression.** Select `:Wise1934` for overhead placement and `:Xue2018`
...
"""
description(::Formula{:default}) = "Select earth impedance from the resolved problem context"
