# Private numerical records. These are not new physical shapes or declarations.
...
_shunt_same(a, b, scale) = spectral_magnitude(a - b) <= _shunt_tol(scale)
...
function _shunt_layers(regions, inner, outer, x, y, ::Type{T}) where {T}
    ...
    for placed in regions
        ...
        push!(layers, (ri = T(shape.ri), ro = T(shape.ro),
...
function _shunt_host_domain(design, blueprint, host_region, design_index, offset, ::Type{T}) where {T}
    ...
    for i in eachindex(tapes), j in 1:(i - 1)
        ...
        radial > _shunt_tol(scale) || angular > _shunt_tol(scale)/_shunt_scalar(scale) || return nothing
    end
...
function _shunt_domain_equal(a, b)
    ...
    for name in (:a, :b)
        earth_state_equal(getproperty(a,name), getproperty(b,name)) || return false
    end
    for name in (:left, :right, :wires, :tapes)
        l, r = getproperty(a,name), getproperty(b,name)
        length(l) == length(r) || return false
        if name in (:left,:right)
            all(earth_state_equal((x.ri,x.ro,x.material.eps_r),(y.ri,y.ro,y.material.eps_r))
                for (x,y) in zip(l,r)) || return false
        else
            all(earth_state_equal(x,y) for (x,y) in zip(l,r)) || return false
        end
    end
    return earth_state_equal(a.material.eps_r, b.material.eps_r)
end
