...
function (formula::Formula{ID})(
    ...
    formula.assumptions.longitudinal === :zero && !iszero(longitudinal) &&
        throw(ArgumentError("earth-potential coefficient :$ID fixes Γ to zero"))
...
function FormulaMethod(formula::Formula{ID}, pair::EarthPair) where {ID}
    return FormulaMethod(Val(ID), earth_potential_coefficient,
        Val(pair.row == pair.column ? :self : :mutual), Val.(pair.layers)...)
end

function earth_potential_coefficient(::Val{ID}, ::Val{Kind}, ::Val{S}, ::Val{T},
        functor, pair, workspace) where {ID, Kind, S, T}
    throw(ArgumentError(
        "earth_potential_coefficient :$ID ($Kind): formula not implemented for source in layer $S and target in layer $T"))
end

const EQUATION_FALLBACK = which(earth_potential_coefficient, Tuple{
    Val, Val, Val, Val, Any, Any, Any})
