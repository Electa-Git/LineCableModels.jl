...
levelfrom(v::Integer)::Logging.LogLevel = v >= 2 ? Logging.Debug :
                                          v == 1 ? Logging.Info :
                                          v == 0 ? Logging.Warn :
                                          Logging.Error
...
function Logging.min_enabled_level(logger::TimestampLogger)
    Logging.min_enabled_level(logger.logger)
end
function Logging.shouldlog(logger::TimestampLogger, level, _module, group, id)
    Logging.shouldlog(logger.logger, level, _module, group, id)
end

function Logging.handle_message(
        logger::TimestampLogger, level, message, _module, group, id,
