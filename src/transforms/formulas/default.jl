...
function modal_operators(
    ...
    work = levenberg_marquardt_workspace(Val(:default), R, n)
    ...
    if fallback_count > 0
        @warn ":default retained matched eigensolutions where LM lost relative accuracy" fallback_count first_fallback
    end
    return _maps(current)
end

function computation_options(::FormulaMethod{:default, typeof(modal_operators)})
    (iteration = (
