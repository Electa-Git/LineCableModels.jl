...
Select conditional Monte Carlo propagation over a [`ParametricProblem`](@ref).
Randomness is local and reproducible when `seed` is supplied.
...
for ResultType in (:ParametricResult, :LinearErrorResult, :MonteCarloResult)
    @eval begin
...
result(value::Union{ParametricResult, LinearErrorResult, MonteCarloResult}) = value.values
...
function manifest(value::Union{ParametricResult, LinearErrorResult, MonteCarloResult})
    value.details[:manifest].value
end
