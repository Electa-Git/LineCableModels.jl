...
function _mc_selection(
    ...
    sample_values = result.samples === nothing ? nothing : collect(
...
_mc_empirical_cdf(sorted_values, x) = searchsortedlast(sorted_values, x) /
...
PlotBuilder.input_defaults(::Type{MCDistributionPlotSpec}, ::MonteCarloResult) =
    _mc_input_defaults()
...
function PlotBuilder.resolve_input(
    ...
    input.nbins === nothing || input.nbins isa Int || throw(
        ArgumentError("nbins must be an integer or nothing"),
    )
    input.nbins === nothing || input.nbins > 0 || throw(
    ...
    needs_samples && values === nothing && throw(
