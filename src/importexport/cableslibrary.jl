...
function _load_cableslibrary_json!(library::CablesLibrary, file_name::String)
    ...
    elseif haskey(json_data, "__julia_type__") &&
        ...
        designs_to_process = json_data["data"]
    elseif all(
        v -> v isa AbstractDict && haskey(v, "__julia_type__") &&
    ...
    for (cable_id, design_data) in designs_to_process
        ...
        try
            ...
            reconstructed_design = _reconstruct_cabledesign(string(cable_id), design_data)
...
Helper function to reconstruct a [`ConductorGroup`](@ref) or [`InsulatorGroup`](@ref) object with the first layer of the respective `AbstractCablePart`. Subsequent layers are added using `add!` methods.

# Arguments
- `layer_data`: Dictionary containing the data for the first layer, parsed from JSON.

# Returns
- A reconstructed [`ConductorGroup`](@ref) object with the initial `AbstractCablePart`.
...
function _reconstruct_partsgroup(layer_data::Dict)
    ...
    try
        ...
        elseif LayerType == Sector
            ...
            ismissing(params) &&
                Base.error("Missing 'params' for Sector in data: $layer_data")
            !(params isa SectorParams) &&
                error("'params' did not deserialize to a SectorParams object. Got: $(typeof(params))")
            ismissing(rotation_angle_deg) &&
                Base.error("Missing 'rotation_angle_deg' for Sector in data: $layer_data")
        ...
        elseif LayerType == SectorInsulator
            ...
            ismissing(inner_sector) &&
                Base.error("Missing 'inner_sector' for SectorInsulator in data: $layer_data")
            !(inner_sector isa Sector) &&
                error("'inner_sector' did not deserialize to a Sector object. Got: $(typeof(inner_sector))")
            ismissing(thickness) &&
                Base.error("Missing 'thickness' for SectorInsulator in data: $layer_data")
...
function _reconstruct_cabledesign(
    ...
    cable_design = CableDesign(cable_id, reconstructed_components[1]; nominal_data = nominal_data)
