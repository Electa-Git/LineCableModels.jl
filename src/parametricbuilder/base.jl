...
function _arg_choice_count(a)
    (a isa Tuple && length(a) == 2) ? (_choice_count(a[1]) * _choice_count(a[2])) :
    _choice_count(a)
end

function _args_choice_count(args::Tuple)
...
function Base.show(io::IO, ::MIME"text/plain", cbs::CableBuilderSpec)
    ...
    for cname in comp_names
        ...
        c_mat = _choice_count(p1c.material)
        for pc in cond[2:end]
            c_dim *= (pc.dim == p1c.dim) ? 1 :
                     (_choice_count(pc.dim[1]) * _choice_count(pc.dim[2]))
        ...
        i_mat = _choice_count(p1i.material)
        for pi in insu[2:end]
            i_dim *= (pi.dim == p1i.dim) ? 1 :
                     (_choice_count(pi.dim[1]) * _choice_count(pi.dim[2]))
...
function _earth_choice_count(e::EarthSpec)
...
function _fmt_freqs(f::AbstractVector)
    length(f) ≤ 8 ? join(f, ", ") :
