...
function EarthSpec(; rho, eps_r = 1.0, mu_r = 1.0, t = Inf)
    EarthSpec(_spec(rho), _spec(eps_r), _spec(mu_r), _spec(t))
end

function Earth(; rho, eps_r = 1.0, mu_r = 1.0, t = Inf)
    EarthSpec(_spec(rho), _spec(eps_r), _spec(mu_r), _spec(t))
end
...
function SystemBuilder(id::AbstractString, cbs::CableBuilderSpec,
        positions::AbstractVector{<:AbstractPositionSpec};
        length = 1000.0, temperature = 20.0, earth::EarthSpec, f::AbstractVector{<:Real})
    SystemBuilderSpec(id, cbs, positions; length, temperature, earth, f)
end
...
function _expand_earth(e::EarthSpec)
    (
        (ρ, ε, μ, t)
...
function iterate(spec::SystemBuilderSpec)
    return Channel{LineParametersProblem}(32) do ch
        ...
        try
            ...
            @debug "iterate SystemBuilderSpec finished" produced=produced upper_bound=cardinality(spec)
        end
    end
end
