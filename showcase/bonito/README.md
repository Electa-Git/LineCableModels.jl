...
```sh
julia --threads=auto --project=showcase/bonito showcase/bonito/serve.jl
```
...
`presentations/ICHQP2026/content/030_cableconstants.jl` is a package-integration
deck. Its one generic builder creates both nominal `CableDesign{Float64}` and
uncertain `CableDesign{Measurement{Float64}}` values through the public physical
tree. Package-resolved preview polygons feed persistent Makie layer and batched
strand polygon plots; slider changes replace plot data without replacing the
figure, axis, canvas, or plot objects. A separate latest-only worker evaluates one
frequency point through `CableConstants` and publishes the package's R/L/C/G
observations atomically after a short debounce.

LineCableModels is pinned to public upstream commit
`b2b074712f291fac89fd2f4936724c565afa3953`, which supports cable constants
for an insulated core without manufacturing an outward grounded conductor.
The toy design therefore contains only the stranded copper core and XLPE.
The showcase contains no copied impedance, capacitance, or
uncertainty-propagation implementation.

## ICHQP2026 line parameters

`presentations/ICHQP2026/content/040_lineparameters.jl` builds two public
LineCableModels cable designs, places their core terminals one metre below the
earth interface, and evaluates one `LineParametersProblem` from 0.5 Hz through
the 50th harmonic. Cable-1 geometry, cable spacing, earth resistivity, and line
length are rebuilt as `Measurement` inputs after a short latest-only debounce.
The package's default formula bundle owns the physics; the deck requests total
basis and reads R/L/C/G through `observe`.

One result updates persistent 2×2 Makie dashboards for the self (1,1) and
mutual (1,2) matrix elements. Both pages share session-local inputs and a
single numerical worker. Home activation compiles the full sweep and WGL scene
before the deck is opened.

## ICHQP2026 application case

`presentations/ICHQP2026/content/050_applicationcase.jl` reuses the
PowerImpedance OHL/UGC transition model from the live manual. One prepared
power flow supplies a session-local frequency model whose passive-element
closures remain attached to that session's mutable OHL and UGC lengths. Slider
updates therefore evaluate deterministic lower, nominal, and upper length
cases without `Measurements.jl` and without repeating the power flow.

Three persistent Makie axes display the B5 harmonic-impedance boundaries. The
network diagram is also persistent: the blue B4–BX UGC segment grows while the
red BX–B5 OHL segment contracts, and its component-glyph legend is generated by
the presentation-local PowerImpedance diagram extension.
...
browser tab. Page changes within a deck use fragments. Cross-deck navigation
updates browser history and lazily swaps a Bonito deck subsession inside the
mounted application shell. The shell remains the browser's fullscreen element,
so both kinds of transition preserve native fullscreen. Direct deck URLs,
refresh, and browser back/forward navigation remain supported.
...
No GitLab key is required: PowerImpedance and the cable-constants-capable
LineCableModels revision are both fetched from public GitHub.
