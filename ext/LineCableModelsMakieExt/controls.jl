...
function LineCableModels.axisscale!(p::LineCableModels.UIPlot,dimension::Symbol,scale;panel=nothing)
    ...
    entries=filter(entry -> entry.axis in axes && _addon_numeric_axis(entry.axis,dimension),p.addon_state.axis_bindings)
    isempty(entries) && throw(ArgumentError("no eligible numeric $dimension axis in the selected panels"))
...
_addon_expected_callback_error(error)=error isa Union{ArgumentError,DomainError,BoundsError,DimensionMismatch,SystemError,Base.IOError}

function _addon_widget!(builder,p,key;event=nothing,callback=nothing,success=nothing,standard=false)
    state=p.addon_state
    state.controls_enabled || throw(ArgumentError("this figure was constructed with controls=false"))
    ...
    standard || key ∉ _ADDON_STANDARD_CONTROLS || throw(ArgumentError("widget key $key is reserved"))
    (event===nothing)==(callback===nothing) || throw(ArgumentError("event and callback must be supplied together"))
...
function LineCableModels.addwidget!(builder,p::LineCableModels.UIPlot,key::Symbol;event=nothing,callback=nothing,success=nothing)
...
function LineCableModels.removewidget!(p::LineCableModels.UIPlot,key::Symbol)
    key ∉ _ADDON_STANDARD_CONTROLS || throw(ArgumentError("standard widget $key cannot be removed"))
    haskey(p.addon_state.widget_bindings,key) || throw(ArgumentError("unknown widget $key"))
    return _addon_edit_presentation!(p) do
...
function _addon_controls!(p,xsetters,ysetters)
    ...
    if !isempty(p.axes)
        _addon_widget!((p,slot) -> Button(slot;label=_addon_icon(_ADDON_REFRESH_ICON),
                width=_ADDON_BUTTON_SIZE,height=_ADDON_BUTTON_SIZE,buttoncolor=_ADDON_BUTTON_BACKGROUND),p,:reset;
            event=button -> button.clicks,callback=(p,_) -> LineCableModels.resetview!(p),success="Axis limits reset",standard=true)
    end
    if Base.get_extension(LineCableModels,:LineCableModelsCairoMakieExt)!==nothing
        _addon_widget!((p,slot) -> Button(slot;label=_addon_icon(_ADDON_SAVE_ICON),
                width=_ADDON_BUTTON_SIZE,height=_ADDON_BUTTON_SIZE,buttoncolor=_ADDON_BUTTON_BACKGROUND),p,:export_svg;
    ...
    for (dimension,entries) in ((:x,xsetters),(:y,ysetters))
        ...
        toggle=_addon_widget!(p,key;event=control -> control.active,standard=true,
                callback=(p,enabled) -> begin
                    ...
                    changing[]=true
                    try
                        eligible=filter(entry -> getproperty(entry.axis,Symbol(dimension,:scale))[] in
                            (identity,log10,_addon_scale(:pseudolog10)),entries)
                        _addon_set_axis!(eligible,dimension,enabled ? :log10 : :linear)
                        p.status[]=enabled ? "$dimension-axis logarithmic view" : "$dimension-axis linear view"
                    catch
        ...
        for entry in entries
            subscription=on(toggle.blockscene,getproperty(entry.axis,Symbol(dimension,:scale));update=true) do _
                kinds=unique(getproperty(item.axis,Symbol(dimension,:scale))[]===log10 ? "log" :
                    getproperty(item.axis,Symbol(dimension,:scale))[]===identity ? "linear" :
                    getproperty(item.axis,Symbol(dimension,:scale))[]===_addon_scale(:pseudolog10) ? "signed log" : "custom" for item in entries)
