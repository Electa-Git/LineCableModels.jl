...
function _addon_frame_size(axis, allocation=Tuple(axis.layoutobservables.computedbbox[].widths))
    ...
    aspect=axis.aspect[]
    ratio=aspect isa DataAspect ? Float64(axis.finallimits[].widths[1]/axis.finallimits[].widths[2]) :
...
function _addon_axis_insets(axis)
    ...
    sides=ntuple(4) do i
        ...
        mode isa Mixed || return 0.
        side=getfield(mode.sides,i)
        return side isa Real ? Float64(getfield(native,i)+side) : 0.
    end
...
function _addon_axis_span(axis,frame)
    ...
    return ntuple(2) do d
        ...
        (inner+insets[d])/(size isa Relative ? size.x : 1.)
    end
end
...
function _addon_fit_frames!(p,frames;canvas=nothing)
    ...
    try
        if state.panel_page!==nothing
            ...
            widths=zeros(columns);heights=zeros(rows)
            for data in values(state.panel_data)
                panel=data.panel.layout
                frame=_addon_axis_span(data.axis,frames[data.axis])
                panel.width[]=Auto();panel.height[]=Auto()
                panel.tellwidth[]=true;panel.tellheight[]=true
                colsize!(panel,2,Fixed(frame[1]));rowsize!(panel,2,Fixed(frame[2]))
            end
            _addon_compose_guides!(p)
            for data in values(state.panel_data)
                panel=data.panel.layout
                ...
                panel.width[]=bounds[1];panel.height[]=bounds[2]
            ...
            if !isempty(state.frame_padding)
                widths.=maximum(widths);heights.=maximum(heights)
            end
            for column in 1:columns
                colsize!(shell.canvas,column,Fixed(iszero(widths[column]) ? maximum(widths) : widths[column]))
            end
            for row in 1:rows
                rowsize!(shell.canvas,row,Fixed(iszero(heights[row]) ? maximum(heights) : heights[row]))
            end
            shell.canvas.width[]=Auto();shell.canvas.height[]=Auto()
        else
            ...
            canvas===nothing && (canvas=Tuple(shell.canvas.layoutobservables.computedbbox[].widths))
            shell.canvas.width[]=canvas[1];shell.canvas.height[]=canvas[2]
        end
        ...
        rowsize!(shell.root,body_row,Auto(true));colsize!(shell.root,1,Auto(true))
        _addon_compose_guides!(p)
        required=_addon_required_size(shell.root)
        # Width settles measured wrapping before the native required height.
        _addon_resize_preserving_views!(p,(required[1],p.figure.scene.viewport[].widths[2]))
        required=_addon_required_size(shell.root)
...
function _addon_release_frames!(p)
    state=p.addon_state;shell=state.shell
    if state.panel_page!==nothing
        for data in values(state.panel_data)
            panel=data.panel.layout
            panel.width[]=Relative(1);panel.height[]=Relative(1)
            panel.tellwidth[]=false;panel.tellheight[]=false
            colsize!(panel,2,Auto(false,1));rowsize!(panel,2,Auto(false,1))
        end
        for row in 1:size(shell.canvas)[1];rowsize!(shell.canvas,row,Auto(false,1));end
        for column in 1:size(shell.canvas)[2];colsize!(shell.canvas,column,Auto(false,1));end
    end
    shell.canvas.width[]=Auto();shell.canvas.height[]=Auto()
    shell.canvas.tellwidth[]=false;shell.canvas.tellheight[]=false
    shell.body.width[]=Auto();shell.body.height[]=Auto()
    shell.body.tellwidth[]=false;shell.body.tellheight[]=false
    colsize!(shell.body,2,Auto(false,1));rowsize!(shell.body,2,Auto(false,1))
...
function _addon_calibrate_frames!(pages,capacity)
    ...
    for p in pages
        ...
        for axis in p.axes
            ...
            inner=ntuple(2) do d
                size=getproperty(axis,d==1 ? :width : :height)[]
                size isa Real ? Float64(size) : frames[d]*(size isa Relative ? size.x : 1.)-insets[d]
            end
...
function _addon_edit_presentation!(action,p; before=nothing)
    ...
    (;frames,canvas,views,protrusions)=before===nothing ? _addon_frame_snapshot(p) : before
    state.fitting_geometry[]=true
    try
        result=action()
        _addon_compose_guides!(p)
