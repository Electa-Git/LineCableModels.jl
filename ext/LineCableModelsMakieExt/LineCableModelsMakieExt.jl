...
const _PrimarySource = Union{LineCableModels.LineParameters,LineCableModels.SeriesImpedance,LineCableModels.ShuntAdmittance,LineCableModels.CableConstants}
...
function plot(observed::AbstractVector{<:Grammar.ObservedResult},selection=nothing;
        ...
        series_labels=nothing,series_attributes=nothing,problem=nothing,formulations=nothing,band=nothing,
    ...
    errorbar_sampling===nothing || errorbar_sampling in (:staggered,:all) || throw(ArgumentError("errorbar_sampling must be :staggered or :all"))
    ...
    for (page_index,page) in enumerate(pages)
        ...
        styles=_addon_comparison_styles(Tuple(slots[i] for i in retained),Tuple(roles[i] for i in retained),length(observed))
        position=legend_position===_omitted ? (length(retained)>1 || explicit_labels ? :bottom : nothing) : legend_position
        sampling=errorbar_sampling===nothing ? (length(retained)>1 ? :staggered : :all) : errorbar_sampling
        ...
        title===nothing && title_prefix!==nothing && !ismissing(title_prefix) && (page_title="$title_prefix — $automatic_title")
        ...
        push!(built,with_theme(_addon_theme(export_theme=export_theme)) do
            _addon_semantic_line_page(first(sources),Tuple(published[i] for i in retained),
        ...
        last(built).addon_state=merge(last(built).addon_state,(observed=sources,
            display_groups=((request=requests[request_index],groups=groups[request_index]),),
...
Makie.plot(source::Grammar.ObservedResult,args...;kwargs...) = plot(source,args...;kwargs...)
Makie.plot(source::AbstractVector{<:Grammar.ObservedResult},args...;kwargs...) = plot(source,args...;kwargs...)

function plot(sources::Union{AbstractVector{<:_PlotSource},Tuple{Vararg{_PlotSource}}},selection=nothing;
...
Makie.plot(sources::Union{AbstractVector{<:_PlotSource},Tuple{Vararg{_PlotSource}}},args...;kwargs...) = plot(sources,args...;kwargs...)
...
function plot(first::_PrimarySource,second::_PrimarySource,rest...;ydata=nothing,kwargs...)
...
Makie.plot(first::_PrimarySource,second::_PrimarySource,rest...;kwargs...) = plot(first,second,rest...;kwargs...)

function plot(sources::NamedTuple,selection=nothing;ydata=nothing,series_labels=nothing,kwargs...)
    all(source -> source isa _PlotSource,values(sources)) || throw(ArgumentError("named result plotting requires supported primary or observed members"))
...
Makie.plot(sources::NamedTuple{K,<:Tuple{Vararg{_PlotSource}}},args...;kwargs...) where K = plot(sources,args...;kwargs...)
