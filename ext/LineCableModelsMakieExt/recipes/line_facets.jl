...
function _semantic_line_facets(published, ydata)
    ...
    for request_index in eachindex(ydata)
        ...
        extent=c.kind===:matrix ? ntuple(d -> maximum(source.observations[request_index].coordinates.extent[d]
            for source in published),length(c.extent)) : c.extent
        for (local_row, row) in enumerate(rows), (local_column, column) in enumerate(columns)
            push!(facets, (; request_index, local_row, local_column, row,
...
function _semantic_line_pages(facets, capacity)
    pages=NamedTuple[]
    for request_index in unique(facet.request_index for facet in facets)
        ...
        nr,nc=first_facet.extent[1:2]
        br,bc=capacity
        for ir in 1:cld(nr,br), ic in 1:cld(nc,bc)
            retained=filter(f -> cld(f.row,br)==ir && cld(f.column,bc)==ic,selected)
            isempty(retained) && continue
            positions=Tuple((mod1(f.row,br),mod1(f.column,bc)) for f in retained)
            dimensions=(min(br,nr-(ir-1)*br),min(bc,nc-(ic-1)*bc))
            push!(pages,(;facets=retained,positions,dimensions,index=(ir,ic)))
        end
    end
    return pages
end
...
function _semantic_quantity_title(object, facet)
    ...
    coordinate=facet.domain===:ModalDomain ? "mode" : "conductor"
    facet.row==facet.column && return "$label, $coordinate $(facet.row)"
    return "$label, $(coordinate)s $(facet.row) → $(facet.column)"
end
...
function _addon_semantic_line_page(
    ...
    # Actual domain tracks preserve selection holes but exclude residual padding.
    ...
    for (panel_index, (facet, position)) in enumerate(zip(page.facets, page.positions))
        ...
        axis,scales = _addon_axis!(
            ...
            xscale=xscale===nothing ? (facet.kind in (:matrix,:diagonal) ? :log10 : :linear) : xscale,
            yscale,
            xlabel=get(xobservation,:label,nothing),
            attributes=facet.kind===:assemblies ? merge(attributes,(dim1_conversion=Makie.CategoricalConversion(),)) : attributes,
        ...
        for source_index in eachindex(published)
            ...
            binding=(;
                xdata = facet.kind===:assemblies ? nothing : source.frequencies[facet.request_index].values,
                ydata = curve,
                yerror,
                interval_support,
                sampled_intervals = facet.kind in (:matrix,:diagonal) && errorbar_sampling === :staggered,
                plots
            )
            ...
            push!(series,merge(binding,(full_support=(x=support(:x),y=support(:y)),)))
        end
    ...
    local_panel_legends=Tuple(pair for pair in _addon_panel_legend_pairs(panel_legends) if first(pair) in selected)
    built = _addon_finish!(
    ...
    built.addon_state=merge(built.addon_state,(panel_page=(;
        index=page.index,dimensions=page.dimensions,
        coordinates=Tuple((f.row,f.column) for f in page.facets)),page_cells=cells,))
    return built
end
