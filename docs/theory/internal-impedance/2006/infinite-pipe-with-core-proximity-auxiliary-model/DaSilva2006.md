...
`PipeImpedance.Formula(:DaSilva2006; wall=:infinite)` supplies
the Method 2 core-to-pipe coefficients. It requires an infinite
outer-radius argument and physical solid-core data through
`PipeImpedance.with_cores`. The individual core-skin term remains
a separate coaxial contribution.

The finite and infinite wall responses share their circular geometry,
but retain different boundary coefficients. The core-proximity
coefficient (4) is the same directional series as [Kane1995](@cite),
(10) and (17). The self sum includes every other core, following
Kane (9) and the source's count of other conductors; the printed
``P-1`` bound is not used to omit one conductor according to its label.

Reciprocal matrix use requires equal solid-core radii and material
values. Unequal-core directional terms remain available through
the separate pair evaluator and are not symmetrized.

An infinite wall has no finite outer-surface or through-wall transfer
term. Requests for those terms, or substitution into a finite-wall
exterior assembly, are rejected. The implemented coefficients remain
usable in the source's core-to-pipe loop coordinates.
