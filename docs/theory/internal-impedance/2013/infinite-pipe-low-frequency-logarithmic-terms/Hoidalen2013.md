...
`PipeImpedance.Formula(:Hoidalen2013; wall=:infinite)` evaluates
the low-frequency inner response and pipe coupling in core-to-pipe
coordinates. It requires an infinite outer-radius argument and
provides no finite outer-surface or transfer coefficient. It is not
a low-frequency replacement for a physically finite wall.

The sign of the first harmonic correction follows directly from the
unexpanded parent (3). Set ``L=\ln(2/x_1)-\gamma``. Its first coefficient
has the low-frequency form

```math
\frac{2\mu_{rp}}{1+\mu_{rp}+x_1^2L}
=
\frac{2\mu_{rp}}{1+\mu_{rp}}
-
\frac{2\mu_{rp}}{1+\mu_{rp}}
\frac{x_1^2L}{1+\mu_{rp}+x_1^2L}.
```

Thus the numerical interpretation subtracts the positive correction
printed in (16), while the plus sign in the transcription of (15)
remains unchanged. The remaining harmonics use the static logarithm.
The inner term retains only the leading logarithm in (14); no
higher-order accuracy or universal frequency threshold is claimed.

The incomplete mixed-model diagnostic (23) is not needed for these
coefficients and is not supplied as another formula.
