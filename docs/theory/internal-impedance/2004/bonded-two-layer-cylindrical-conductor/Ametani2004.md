...
```math
Z_{out}=\frac{z_{2o}-z_{2m}^{2}}{z_{1o}+z_{2i}},
\qquad\text{(9), as printed}
```

```math
\begin{aligned}
Z_{in}&=\frac{z_{1i}-z_{1m}}{z_{1o}+z_{2i}} \\
Z_m&=\frac{z_{1m}z_{2m}}{z_{1o}+z_{2i}},
\end{aligned}\qquad\text{(10), as printed}
```
...
\end{aligned}\qquad\text{(A.11)}
```

```math
\begin{aligned}
m_n^2&=j\omega\mu_n\eta_n-\Gamma^2 \\
\eta_n&=\sigma_n+j\omega\varepsilon_n, \\
m_n^2&\simeq j\omega\mu_n\sigma_n=\frac{j\omega\mu_n}{\rho_n}
\quad\text{under (A.12).}
\end{aligned}\qquad\text{(A.2--A.3)}
```
...
The executable reduction follows the complete terminal matrix (5)–(8)
and the interface conditions, not the misplaced fraction bars in (9)
and the first line of (10). For two bonded materials it is
...
These expressions also match [Ghosh2019](@cite), (11),(16)–(21),
and [Ghosh2022](@cite), (9)–(11). The original printed forms remain
above. The numerical geometry requires a shared interface ``b=b'``;
a dielectric gap is not treated as a bond. The good-conductor
restriction and the stated penetration-depth condition remain part of
the model. No propagation-dependent appendix evaluator is implied.

The coaxial assembler retains physical metal radii and materials before
homogenization. It combines attached semiconducting layers at their
actual interfaces and evaluates the remaining insulating intervals
separately. A semiconductor included in an effective surface is not
also counted as a magnetic insulation interval. All physical radial
shunt-layer contributions remain present.

The same interface elimination can be repeated across bonded layers.
The single-material case reduces to Schelkunoff, and the dc outer
resistance is the parallel combination of the physical layer
resistances. Guard precision controls cancellation when semiconductor
resistance greatly exceeds metal resistance; the final surface terms
retain the requested numeric type.
