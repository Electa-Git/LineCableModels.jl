...
For implementation, derive the sequence quantities by Kron reduction of the published ``7\times7`` impedance matrix. For its symmetric sheath block, the zero-sequence reduction is

```math
z_0=z_{aa}+2z_{ab}
-\frac{(z_{a1}+2z_{a2})^2}{z_{11}+2z_{12}}.
```
