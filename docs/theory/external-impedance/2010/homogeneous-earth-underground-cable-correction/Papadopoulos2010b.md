...
| --- | --- | --- |
| Impressed longitudinal propagation constant ``Γ`` | Source symbol ``\gamma_x`` with longitudinal factor ``e^{-\gamma_x x}``; for the displayed final expression the source sets ``\gamma_x \simeq j k_x = j\omega\sqrt{\mu_1\varepsilon_1}``. | Stated — printed p. 962, text below (3b) and preceding (4). |
| Air propagation constant ``γ_air`` | ``\gamma_0^2=j\omega\mu_0(\sigma_0+j\omega\varepsilon_0)`` under the source's indexed definition; air is labelled only by ``\mu_0,\varepsilon_0``, implying ``\sigma_0=0``. It is retained through ``\alpha_0``. | Equation-implied — printed p. 962, definition below (2b), Fig. 1, and (5b). |
| Earth propagation constant ``γ_earth`` | ``\gamma_1^2=j\omega\mu_1(\sigma_1+j\omega\varepsilon_1)``; retained through ``\alpha_1``. | Stated — printed p. 962, definition below (2b). |
| Earth permittivity and displacement current | ``\varepsilon_1`` is retained in ``\gamma_1``. | Stated — printed pp. 962–963, propagation-constant definition and discussion following (6c). |
...
**Approximation.** The dipole-field starting point is the full electromagnetic-field solution, but the displayed (5) follows the explicit lossless longitudinal-propagation approximation ``\gamma_x\simeq j\omega\sqrt{\mu_1\varepsilon_1}`` and the substitution ``u^2-k_x^2=\lambda^2`` using identity (4). No series truncation is applied to (5); its semi-infinite integral is evaluated numerically.
...
The evaluator retains the earth permeability in the prefactor of (5a).
Its interface integral uses
``1/[\alpha_1+(\mu_1/\mu_0)\alpha_0]``, obtained by dividing
the second addend of (5b) by ``2\mu_0``. The older
``1/(\alpha_0+\alpha_1)`` form and air-permeability prefactor
are recovered only when ``\mu_1=\mu_0``.

The direct-minus-image integral is evaluated as
``K_0(\alpha_1(0)d)-K_0(\alpha_1(0)D)``. The default stored
longitudinal quantity is the real ``k_x``, not the source's
``\gamma_x=jk_x``; an explicit numerical `Γ` replaces
``k_x`` in ``\alpha_k^2=\lambda^2+\gamma_k^2+k_x^2``.
The outgoing square-root branch follows the frequency sign.

Source-kernel tests cover both equal and unequal permeabilities,
the stated default and explicit longitudinal values, radius-substituted
self terms, negative-frequency conjugacy, and complete matrix assembly.
...
- The source calls ``\gamma_x\simeq j\omega\sqrt{\mu_1\varepsilon_1}`` a high-frequency approximation but gives no formal bound; the numerical test range does not define its validity domain.
