...
The executable coefficient follows the final (14), (15), and homogeneous
reduction, independently of the appendix transcription defects. It supports
one to three earth layers. The reflection ratio is evaluated with the finite
thicknesses ``d_1`` and ``d_2-d_1``, so no growing exponential is formed.
The positive-real transverse root supplies decay into a passive earth;
the negative-frequency value is its complex conjugate.

For self assembly, the earth correction uses the source's own-image
geometry ``y=0,H=2h``. The engine adds the usual thin-wire ideal-ground
term ``j\omega\mu_0\ln(2h/a)/(2\pi)`` to that correction. This assembly
convention is separate from the source transcription and does not claim
an exact finite-radius solution. The image correction and literal final
coefficients are tested separately for unequal layer depths and permeabilities.
