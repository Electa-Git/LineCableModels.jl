...
**Limitations.** Only a single bare conductor and one interface are covered. Multiple modal roots and a fast-wave/TL-mode transition can make full-wave root selection discontinuous. The image-derived qFW estimate avoids that iteration but does not establish a universal frequency range.
...
The qFW selection uses Appendix D, not the derivative in (9). The full-wave modal iteration and its fast-wave/TL-mode selection remain outside the implemented scalar coefficient.
