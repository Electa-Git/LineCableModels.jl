...
**Numerical evaluation.** The source parent (9a) requires the nonzero
``n=0`` term. The printed lower bound ``n=1`` is retained above as a
source transcription; numerical evaluation starts at zero to recover
that parent. The denominator is ``2^n n!(2n-1)``, as printed in both
(5) and (16). The former ``2^{2n}`` transcription is corrected.
