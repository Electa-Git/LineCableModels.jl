...
**Numerical evaluation.** The second term in (6) requires the common
factor ``e^{-kH}`` obtained from the finite parent (22). This factor is
included in the evaluator and checked against (2), (9a), and the first
Bessel series; the printed equation above remains unchanged.
[Pawlik2020](@cite), equation (25), independently retains this exponential
in its corrected later evaluation.
...
| ``J_{\mathrm{Pollaczek}}`` | unchanged | Parent interface integral | dimensionless |
