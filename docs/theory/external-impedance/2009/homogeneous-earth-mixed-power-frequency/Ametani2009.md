...
**Numerical interpretation.** The leading limit of (27) fixes the coefficient without using the rounded printed factors independently:

```math
Z_m^{\mathrm{LF}}=
\frac{j\omega\mu_0}{2\pi}\ln\!\left(\frac{2}{m d}\right),
\qquad
d=\sqrt{(h_a+h_g)^2+y^2},
\qquad
|m|\max(h_a,h_g,d)\ll1.
```

Here ``h_a,h_g>0`` are the overhead height and burial-depth magnitudes. In milliohms per kilometre, the positive-frequency result has the rounded form ``f\{1+j[8.253+0.628\ln(\rho_e/(fd^2))]\}``: the frequency multiplies both parts. Numerical evaluation retains the unrounded SI constants. The printed (31)–(32) above are unchanged; no dimensionless constant is added directly to an impedance. The full Lucca correction remains available in its separate formula.

**Limitations.** The reduction requires penetration depth large relative to the complete pair geometry. It does not replace the exponential image outside that limit.
...
The source explicitly calls (31) identical to a known overhead formula and says the Carson series implemented in EMTP can be used for the mixed arrangement at low frequency with negative buried ``h_2``. This is a distinct published reduction/witness, not a claim of new historical priority. The numerical reduction is selected as an approximation of the existing Ametani2009 formula, without a second publication identifier.

## Limitations and discrepancies

The source's displayed equations are preserved. Numerical use follows the parent-derived SI reduction above, with signed physical coordinates converted to positive height/depth magnitudes. The source supplies no uniform error bound for that reduction.
