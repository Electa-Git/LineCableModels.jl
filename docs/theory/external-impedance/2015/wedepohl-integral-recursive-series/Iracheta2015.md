...
**Numerical evaluation.** The implementation uses the moment definitions
(5a)–(5b), which fix the duplicated initializer and the even/odd recurrences
through integration by parts. The computed moment combination is inserted
into the checked Pollaczek decomposition [Theodoulidis2012](@cite)
(equation (22)); inconsistent prefactors in the printed restatement (3c)
are not substituted for that parent. The source's zero-residual choice
for ``|D/p|>30`` is retained. These numerical choices do not alter
the source transcription below.
