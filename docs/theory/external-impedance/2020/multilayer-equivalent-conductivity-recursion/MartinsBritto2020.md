...
The recursion retains each layer's permeability in its diffusion constant and returns a real equivalent conductivity. It does not supply an equivalent-permittivity or equivalent-permeability law; those properties remain the selected layer values. The publication's numerical examples concern nonmagnetic soil. Comparisons with rounded tabular examples account for the precision of the published layer inputs.
