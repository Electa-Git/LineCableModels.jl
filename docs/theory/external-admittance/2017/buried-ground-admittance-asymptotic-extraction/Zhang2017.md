...
**Numerical interpretation.** The right-hand side of (11) has potential-coefficient units. It is therefore returned as ``P_g``, and the assembled scalar admittance is ``Y_g'=j\omega/P_g``. Equations (14)–(16) reduce to ``u_0=\lambda`` and ``u_1=\sqrt{\lambda^2+j\omega\mu_0\sigma_g}``, while (13) retains both full bulk propagation constants. These medium-dependent axial references differ from a common prescribed longitudinal wavenumber.

For tail evaluation, the change of variable ``\lambda=Tt`` in (A.8) gives

```math
\int_T^\infty\frac{e^{-c\lambda}\cos(r\lambda)}{\lambda^n}\,d\lambda
=T^{1-n}\Re\!\left[E_n\!\left(T(c+jr)\right)\right],
\qquad c\ge0,\quad n\in\{1,3\}.
```

Here ``E_n`` is the generalized exponential integral defined in (A.12), with its oscillatory boundary value when ``c=0``. The factors multiplying these real moments remain complex. In particular, inverse-cubic moments include ``T^{-2}``. This evaluates the extracted kernel (A.6)–(A.8) in SI units.

The numerical selection provides the unapproximated integral and the extracted-tail alternative. Adaptive quadrature evaluates the finite interval; it does not reproduce the source's separate spline-moment algorithm. The pair radius is the sampling radius. Coaxial assembly supplies the outer earth-interface radius by default; a source-radius selection reproduces the printed metal-core radius ``r`` explicitly. The two radius conventions are not assumed identical. The source defines no mutual coefficient.

**Limitations.** The geometry contains one buried conductor and homogeneous earth. The Vance reduction in (18) requires burial depth much larger than earth skin depth; the integral in (11) does not impose that reduction. The prescribed kernel can yield negative real ground admittance at high frequencies. Normal matrix evaluation rejects those cases; the coefficients are not altered to enforce passivity.
