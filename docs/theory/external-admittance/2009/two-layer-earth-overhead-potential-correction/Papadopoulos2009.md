...
Numerical evaluation uses the final two-layer kernels with the prescribed air propagation reference. The corresponding transverse constants are used consistently in the interface coefficients and finite-layer exponential. Common material-factor scales cancel before evaluating the potential correction. The printed self-radius prescription is retained. Potential coefficients are assembled before the complete matrix inverse.
