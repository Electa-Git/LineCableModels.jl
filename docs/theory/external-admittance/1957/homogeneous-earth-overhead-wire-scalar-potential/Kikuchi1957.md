...
The source describes an infinitely remote return reference and notes that a practical earth-surface reference differs because the earth potential is not spatially constant. The expressions above retain the source's voltage normalization. The charge-normalized numerical coefficient is specified separately below.
...
5. **Actual output and reference.** The scalar-potential kernel is preserved with its published ``\Gamma I/(j\omega\varepsilon_1\pi)`` prefactor. The source defines ``Q`` separately and defines terminal voltage in §9. Charge normalization does not supply a multiwire matrix or convert the vector potential into an impedance formula.
...
Charge continuity supplies the scalar coefficient ``P=V/Q``.
Write ``g_n=j\omega\mu_0(\sigma_n+j\omega\varepsilon_n)``
and map the source longitudinal constant to the engine wavenumber
``k_x=j\Gamma``. Then ``\chi_n^2=g_n+k_x^2`` and
``\lambda_n=j\chi_n`` retain the outgoing first-kind Hankel solution
through the connection
``K_0(\chi)=j\pi H_0^{(1)}(j\chi)/2``.
This is the analytic connection in [DLMF 10.27.8](https://dlmf.nist.gov/10.27.E8);
it does not change the Hankel kind. Fourier roots use positive real
parts and passive lossless continuation.

The source's lower-surface terminal prescription gives

```math
P=\frac{1}{2\pi\varepsilon_1}
\left[
K_0(\chi_1a)-K_0(\chi_1[2h-a])
+2\int_0^\infty
\frac{g_1e^{-(2h-a)q_1}}{g_2q_1+g_1q_2}\,du
\right],
\qquad q_n=\sqrt{u^2+\chi_n^2}.
```

For the separate air-reference approximation (5·9),
``k_x^2=-g_1``, the Bessel difference has the finite limit
``\ln[(2h-a)/a]``. This is the default numerical selection.
A supplied longitudinal wavenumber retains the general scalar
expression without solving a modal dispersion equation. The
nonmagnetic-media and lossless-air restrictions remain enforced.

Only the single overhead-wire terminal coefficient is registered.
The independent observation evaluator retains both (5·1) and (5·2)
and their respective integral exponentials. Its below-interface
observation is not used as a buried-source or mixed-cable coefficient.
Single-wire shunt assembly uses ``Y=j\omega/P``, with any separate
radial potential contribution added before inversion.
