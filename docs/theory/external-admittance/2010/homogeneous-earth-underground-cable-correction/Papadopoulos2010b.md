...
Let ``r=\mu_1/\mu_0``. Combining the interface addends of
``F+G`` gives the half-weight
``(\alpha_0+r\alpha_1)/
[(\alpha_1+r\alpha_0)
(\alpha_0+r(\gamma_0^2/\gamma_1^2)\alpha_1)]``.
This is the algebraic form used by the evaluator. The simpler weight
``1/[\alpha_0+(\gamma_0^2/\gamma_1^2)\alpha_1]`` is
recovered only for equal permeabilities; it is not the general
source formula.

The direct-minus-image term is evaluated through its ``K_0``
identity. The stored longitudinal quantity is the real ``k_x``,
with the source default ``k_x=\omega\sqrt{\mu_1\varepsilon_1}``;
an explicit numerical `Γ` replaces that quantity. The outgoing
root branch follows the frequency sign. Potential coefficients are
assembled with the internal dielectric coefficients before computing
the matrix admittance ``\mathbf Y=j\omega\mathbf P^{-1}``,
as required by Appendix C.

Tests compare the combined weight against independently integrated,
uncontracted source ``F`` and ``G`` terms. They cover unequal
permeabilities, the nonmagnetic reduction, self-radius substitution,
longitudinal inputs, negative-frequency conjugacy, and matrix assembly.
