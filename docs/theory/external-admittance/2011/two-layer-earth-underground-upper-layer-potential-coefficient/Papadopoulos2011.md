...
Numerical evaluation retains the source's upper-earth-layer restriction and physical top-layer thickness. In the potential kernel, the second reflected exponential uses the printed distance 2d+h₂−h₁. Common material-factor scales cancel before quadrature. Independent evaluations of the printed F and G terms check unequal depths, permeability contrast, reciprocity, and negative-frequency conjugacy. Potential coefficients are assembled before the complete matrix inverse.
