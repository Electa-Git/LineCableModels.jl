...
The second half of this calculation already exists:

- [`parallelrc.jl`](/home/amartins/Documents/KUL/LineCableModels-engine/src/engine/insulationadmittance/formulas/parallelrc.jl:30) calculates \(p=s/(G+sC)\).
- [`potential_coefficient`](/home/amartins/Documents/KUL/LineCableModels-engine/src/engine/insulationadmittance/interface.jl:102) adds the physical dielectric layers in radial series.
- [`admittance!`](/home/amartins/Documents/KUL/LineCableModels-engine/src/engine/admittance.jl:25) assembles \(\mathbf P_{\mathrm{ins}}\), then adds the earth potential coefficients.
...
The actual bug is earlier: `Material` owns `tan_delta`, but the workspace copies only layer `rho` and `eps_r` in [`input.jl`](/home/amartins/Documents/KUL/LineCableModels-engine/src/engine/input.jl:281). The loss tangent disappears completely.
...
Keep the existing `InsulationAdmittance` family. No new workflow or problem type is needed.

The common functor should own geometry and radial aggregation. Each registered formula should define the leaf admittivity law:

```julia
κ = admittivity(route, kind, rho, eps_r, tan_delta, frequency, temperature)
p = layerp(r_in, r_ex, κ, s)
```
...
- `:ParallelRC` — preserve the current behavior and numerical baseline

  \[
  \kappa=\sigma_{\mathrm{dc}}+s\varepsilon
  \]

- `:TotalLossTangent` — reported \(\tan\delta\) already includes conduction
...
- `:IntrinsicLossTangent` — \(\tan\delta\) describes polarization loss only
...
For mixed XLPE, semiconductor, and bedding layers, the registered recipe can route its leaf law through `Val(material.kind)`. That preserves one top-level formulation while allowing, for example, conductive semiconductors and measured-loss XLPE within the same cable.
...
Finally, inferring `rho` from a 50/60 Hz \(\tan\delta\) is a legitimate reference-frequency calibration, but `:ParallelRC` then freezes \(G\) while the actual constant-\(\tan\delta\) relation has \(G\propto\omega\). That inferred resistivity should therefore be advertised as a single-frequency equivalent, not a broadband material property.

This was an architectural and mathematical reassessment; I did not change files in this pass.