...
```math
V_1(x,y)=\frac{\Gamma I}{j\omega\epsilon_1\pi}
\left[
\frac{j\pi}{4}
...
```math
V_2(x,y)=\frac{\Gamma I}{j\omega\epsilon_1\pi}(Q'_2-jP'_2),
```
...
```math
k_n=\omega\sqrt{\left(\epsilon_n-j\frac{\sigma_n}{\omega}\right)\mu_0}
...
=-\frac{1}{\epsilon_1}Q\delta(\mathbf r-\mathbf r_0),
\qquad
...
| ``\epsilon_n,\sigma_n`` | Unchanged | Permittivity (F/m) and conductivity (S/m); no added dispersion or dielectric-loss model |
...
- **Air constitutive restriction.** The common bulk definition contains ``\sigma_n``, while the scalar source/prefactor uses ``\epsilon_1`` and ``j\omega`` without an air-conductivity term. The generality of a nonzero ``\sigma_1`` extension is unresolved; none is supplied here.
